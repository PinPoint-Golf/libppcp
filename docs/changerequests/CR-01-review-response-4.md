...
| [3.7h](../specification/ppcp-rv.md#37-the-bootstrap-window) | Reaching a window without multicast | ⛔ **Named by both teams as the difference between a demo and a product, and it is the one open item with no owner.** PinPointCapture has moved it into MVP scope; it is useless unless PinPointStudio does too |

⛔ **3.7h is the item to decide before implementation is planned, not during it.** RV-6 over mDNS works in an office and not at a driving range, which is where CR-01 asked for it. The specification is right to say nothing further — [§11](../specification/ppcp-rv.md#11-rv-6--guided-pairing) constrains the handshake, not how the endpoint was learned — so this is a product decision with no protocol content, and it is the one thing on this list that decides whether the feature is worth building at all.
