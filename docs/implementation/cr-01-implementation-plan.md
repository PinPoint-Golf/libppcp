...
| Unlocks | RT-22 |
| Status | ⛔ **not done — RT-22 FAILS.** C1, PinPointCapture `dbfc262`, `5230596`, `deaac7d`, `539120b`. The window, the second listener, the `bs`/`rn`/`rid` key set and the classifier are all in and green (`make test-core`, 254/254), and the design decisions are right — the window is **its own type, not a flag on the advertiser** ([3.3g](../specification/ppcp-rv.md#33-txt-record)), `tick()` can close a window and can never open one, the listener is ready *before* the window opens, and there is no `bs_offer` parser. ⛔ **But the withdrawal assertion ran for the first time on 24 Aug and failed** — see [log row 12](#10-decisions-findings-and-errata-log). D10 stays open. |
