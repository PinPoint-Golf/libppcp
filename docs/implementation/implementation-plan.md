...
| Status | ☑ **wave 1 done; freeze report pending.** S5 wave 1 (`d043135`…`fce70e2`): **errata E5–E29** written into the normative text with `CORE`'s errata table, `README.md`'s status line and both change histories updated; **E1–E4 verified** present in the clause text and not only in the history. Four questions the package was asked to *decide* are decided and marked reversible (E24–E27, F-D7-1..4). The `CONF` 5b2 sweep of all 45 messages is done and its answer is now a **column in `MSG` §11 that the 5b1 audit asserts on every run**. `ppcp-conform`'s CT-I6 row has a minting-capture counterpart and a `ctest` row of its own. Three cross-team findings from the S5 interop runs closed in-session: **F-S5-3** (library defect + E28), **F-S5-1** (E29 + `ppcp_arbiter_reconsider`), **F-S5-2** (`requesting-host` scenario). 49/49 green; claim regenerated. **Wave 2 — the freeze-readiness report — is outstanding.** |
...
| **S5 — interoperability and freeze** (started 23 Aug; wave 1 = L17 errata + conform CT-I6 fix ∥ H: IOP-4..9 vs `ppcp-sim`, IOP-3/10 import half, TLS listener script ∥ D: IOP-2 vs `ppcp-sim`, IOP-3/10 bundles; wave 2 = the real PPS ↔ PPC pair over TLS on loopback (IOP-1, 6, 10), then L17 freeze report) | L17; run every non-rig interop row in `CONF` §5 with the real pairs (PinPointStudio ↔ `ppcp-sim` as a foreign host; PinPointCapture no-host → bundle → PinPointStudio; PinPointStudio ↔ PinPointCapture on the simulator over loopback) | fixes from interop | fixes from interop | Freeze-readiness report written; errata in the specification; remaining open rows are rig or product decisions |
