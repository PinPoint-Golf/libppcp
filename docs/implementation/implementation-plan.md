...
| Status | **in progress — S3 wave 2.** `tools/ppcp-sim` builds and runs (`5a37cce`). `tools/run-pair.sh SIM LDECL LSCENARIO LEXPECT DDECL DSCENARIO DEXPECT [RUN_MS]` runs two of them over loopback. **Working:** twelve scenarios and ten declarations under `tools/scenarios/`; link binding by `link_bind` on two TCP connections; a full happy-path session (declare → session_open → sync burst per timebase → arm → candidate → shot → capture_announce → payload on the bulk channel); the Core+Live observer refusing `arm` with `profile_not_supported` without closing; a no-Detect host arbitrating over a `candidate`; a silent host leaving the device to mint at the 8.2i deadline; an `unrelated` peer minting nothing; two same-basis Candidates on one Shot; a late Candidate attaching with `t0` unmoved; three timebases producing three probe sequences; a stored Session offered and replayed. `--psk-ke-only` is built behind an optional OpenSSL (tool only). **Next:** the eight paired rows as ctest rows in `tests/CMakeLists.txt`, `tools/scenarios/README.md`, matrix + claim updates. **Three things the first runs surfaced, unresolved:** (i) a replayed bundle's `capture_announce` does not raise `PPCP_EVENT_CAPTURE` at the receiver although its payload frames arrive — under investigation, possibly a `ppcp_bundle_replay` or ingest defect; (ii) in the `late-host` scenario the device did not mint before the delayed host issued, so 8.2k's attach-rather-than-issue was not exercised; (iii) `preview-capture` opens its Streams but announces nothing, because that scenario nominates no Candidate |
