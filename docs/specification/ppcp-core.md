...
- **(5.4d)** *Erratum E67, 2 September 2026.* An `affine` relation is **exact in both directions**, and a peer holding A→B MAY convert a B instant into A through its inverse: `offset' = −offset`, `skew' = −skew / (1 + skew)`, `observed_at' = observed_at + offset` expressed in B, both sigmas unchanged. This is **not** composition — nothing passes through a third timebase and nothing is invented; it is one measurement read from its other end. Where a peer holds relations for the same pair in **both** directions (each of two peers publishes its own estimate of the same two clocks, [§6.3](#63-clock-synchronisation)), a conversion resolves to whichever carries the smaller sigma **at the instant being converted** ([5.4a](#54-timebaserelation)'s two sigmas, evaluated over the elapsed time since `observed_at`). An `unrelated` declaration in **either** direction still means no mapping (5.4b) and is not overruled by a fit the other way. The declared relation on the wire is unchanged by this clause; it governs what a receiver does with what it holds.
...
| `reason` | `Kind` | `available == false`: 1 | Open registry — `in_use`, `permission_denied`, `disconnected`, `thermal_limit`, `storage_full`, … |
...
- **(5.20d)** *Erratum E64, 26 August 2026 — CR-02 review round 1, PinPointCapture.* `no_source` is deliberately absent from `reason`'s example list, though it is `Readiness.blocked_reason`'s vocabulary ([§5.15](#515-readiness)) and would otherwise be the obvious value to carry over. `device_status` is emitted only for an already-declared Source ([`PPCP-MSG` 5.5c](ppcp-messages.md#55-device_status)), so "there is no Source" cannot occur for the event that names one by construction. It remains meaningful for `Readiness`, which is reported at arm time and can legitimately have nothing to arm.
...
- **(6.3c1) SHOULD** *Erratum E68, 2 September 2026.* The burst is **spread over one to three seconds**, not fired back to back. 6.3a asks the exchange for a rate as well as an offset, and a rate fitted over a burst that spans a third of a second is noise: measured on every connect, the slope's sigma after a 320 ms burst was ~600,000 ppm and the fit's residual 25–40 ms, so no consumer with a millisecond policy could use the relation until the first maintenance exchange arrived seconds later. Spread, the same 10–20 exchanges yield a relation that stands on its own.
...
- **(6.3f1) SHOULD** *Erratum E69, 2 September 2026.* The minimum-RTT filter admits exchanges to the fit **relative to the lowest RTT seen** — within a margin of it — not as a fixed share of the window by count; and where the estimator keeps a bounded window, it **retires exchanges by quality, not by arrival order** (highest RTT first, with an age bound so the window's span stays current). A share-by-count filter over a window retired oldest-first kept a connect burst taken while the peer was busy (camera warming, channels opening) in the fit until the maintenance cadence had walked past it: half of sixteen noisy exchanges is still noisy, and sixteen clean ones at one per five seconds is eighty seconds — the whole of the "two minutes to converge" this was measured against. Relative to the minimum, the first clean exchange starts excluding the busy ones.
...
- **(6.3g1) SHOULD** *Erratum E68, 2 September 2026.* The first maintenance exchanges after a burst follow **quickly** — on the order of 0.5, 1, 2 and 3 s — and the cadence stays at about one per second **until the fit rests on enough exchanges inside the RTT gate** (6.3f1), bounded to about a minute, before settling to the steady cadence. A peer busy at connect yields no clean exchange until it is quiet; this is what bounds the wait after it is. ⚠ It is the first seconds only. A higher **steady** rate was measured to make convergence worse on a shared radio, where probes contend with preview video for the medium they are measuring; 6.3g's floor of one per five seconds is unchanged.
...
| **I39** | An `actuator_command`, an `actuator_command_ack.state` (where present), and an `actuator_state.state` each carry `on` if and only if the named Actuator's `control` is `on_off`, and `level` if and only if it is `level` — never neither, never both, on all three. *(Added as an erratum after revision 9, [E58](#errata-after-revision-9), CR-02; extended to the response and event forms by [E63](#errata-after-revision-9), CR-02 review round 1 — both teams independently.)* | Actuate |
...
| **E30** | [`PPCP-RV` §11](ppcp-rv.md#11-rv-6--guided-pairing), [1.3c1](ppcp-rv.md#13-where-it-stops), [2f](ppcp-rv.md#2-rendezvous-paths), [9e, 9f](ppcp-rv.md#9-conformance) | **Added, 24 August 2026 — change request [CR-01](../changerequests/CR-01-in-band-pairing.md), granted in part.** **RV-6, guided pairing**: a first pairing between peers that have never met, with no code carried between two screens. A committed X25519 exchange, authenticated by six digits displayed on both peers and affirmed at both — a *comparison*, not a transfer. It produces a `PRK` and the connection closes; `PPCP-RV` §5 and §7 then apply verbatim, so **2c is unweakened rather than excepted** and there is still no unauthenticated rendezvous path. Optional; the pairing code stays REQUIRED. Not a defect report: revision 8 was self-consistent and did not serve a requirement the product acquired (PinPointCapture, 24 August 2026). |
| **E31** | [`PPCP-RV` 3.2c](ppcp-rv.md#32-instance-name), [3.3f, 3.3g](ppcp-rv.md#33-txt-record), [3.4c1](ppcp-rv.md#34-resolvable-identifiers), [§3.7](ppcp-rv.md#37-the-bootstrap-window) | **Added, 24 August 2026 — CR-01.** The **bootstrap window**: a user-opened, single-attempt, ≤180-second service instance carrying `bs` and an optional operator-set label `dl`, and carrying **no `rid`** — so 3.4c's refusal to dial an instance it cannot resolve is scoped to the reconnection path. That refusal, correct for reconnection, was what made a first pairing over discovery impossible. `dl` is a stated privacy trade against 3.3b, bounded by the window's lifetime. |
| **E32** | [`PPCP-RV` 3.5e](ppcp-rv.md#35-who-advertises-and-who-browses) | **Added, 24 August 2026 — CR-01 question 3.** Where a peer's counterpart cannot advertise for reconnection under 3.5d, the peer that **can** SHOULD. 3.5d says only who must *not* advertise, and read with 3.5b's recommendation a deployment could have **neither** end advertising while satisfying every clause — leaving a persisted pairing valid at both ends with no path by which either finds the other, and users a protocol that remembers them and still asks for a code. |
| **E33** | [`PPCP-RV` 7.1](ppcp-rv.md#71-threat-model), [7.2e](ppcp-rv.md#72-handling-the-pairing-secret), [7.4i](ppcp-rv.md#74-persistent-pairings) | **Added, 24 August 2026 — CR-01.** The security model carried into the new path: what the compared string defends against and what it does not — an operator who does not compare, and one guess in 1 048 576 per confirmation; erasure of the ephemeral key material on success **and** on abort; and that a guided pairing is **pairwise**, so 7.4f's ban on persisting a group credential does not reach it. Records that this path removes the photographable secret §5.4.3 named as the leak route — a consequence of the design, not the reason it was granted. |
| **E34** | [`PPCP-RV` 11.4h, 11.4i](ppcp-rv.md#114-frames), [11.6c](ppcp-rv.md#116-derivation), [§10.4](ppcp-rv.md#104-guided-pairing) | **Amended, 24 August 2026 — finding R-01, PinPointStudio, blocking.** RV-6's bootstrap format version `v` was carried on the wire, checked only against the reader's own capability, and **bound into nothing** — an unprotected agility mechanism in a handshake whose own 11.6g argues that a first-contact exchange with agility is a first-contact exchange with a downgrade attack. An initiator now aborts if `bs_accept.v` differs from what it sent, and `v \|\| pk_i \|\| pk_a` is bound into **both** the short authentication string and the confirmation key, so a rewrite surviving the echo check changes the digits in front of the operator as well as failing the MACs. Latent while `1` is the only value; permanent after either team ships. Four §10.4 rows change; `PRK` does not. |
| **E35** | [`PPCP-RV` 11.3d1](ppcp-rv.md#113-roles-and-the-connection) | **Added, 24 August 2026 — finding R-02, PinPointStudio, blocking.** 11.3d serialised the **acceptor** — *"an acceptor that ran ten attempts in parallel would offer an attacker ten draws against one operator confirmation"* — and that reasoning applies unchanged to the initiator, which nothing stated. The natural implementation is the one that breaks it: `dl` exists so a browsing peer that sees four open windows can tell them apart, and a host dialling all four to show the operator a list of candidate numbers forfeits three orders of magnitude of the security bound, **with the operator actively finding the collision**, while doing nothing the document forbade. |
| **E36** | [`PPCP-RV` 11.6b](ppcp-rv.md#116-derivation), [RT-21](ppcp-rv.md#9-conformance) | **Amended, 24 August 2026 — findings R-03 (PinPointStudio) and F-R9-1 (PinPointCapture), measured independently on two libraries.** The clause required aborting on an all-zero X25519 output that **neither implementation's library ever produces**: OpenSSL 3.6.3 fails `EVP_PKEY_derive` for every small-order point, CryptoKit throws. An implementer following it literally writes a branch that cannot fire and leaves the one that does on the generic transport-error path. The requirement moves to the outcome — abort where the agreement **fails or** yields zeros, and never retry it. E23's shape, in a section written by someone who had just documented E23. |
| **E37** | [`PPCP-RV` 11.4f](ppcp-rv.md#114-frames) | **Reasoning replaced, 24 August 2026 — finding R-04, PinPointStudio. The clause stands.** It said a failed confirmation MAC means an attacker forged one. **An interposed attacker holds the shared secret on both legs and forges both MACs correctly** — a MAC failure is evidence that no such attacker is present, and is overwhelmingly an implementation disagreement. The MAC is an agreement-and-liveness proof; the *comparison* is the authentication. The inverted reasoning would have led an implementer to weigh the two the wrong way round, which is the inversion 11.1d exists to prevent. |
| **E38** | [`PPCP-RV` §11.8](ppcp-rv.md#118-what-the-comparison-proves), [11.2c](ppcp-rv.md#112-why-it-is-not-tls-and-what-that-unlocks) | **Added, 24 August 2026 — finding F-R9-2, PinPointCapture.** 11.2c invited a reviewer to name a value on the plaintext bootstrap connection whose disclosure to a passive observer weakens the pairing; the review took the invitation. The confirmation MACs descend from the shared secret by public functions, so **a recorded transcript is an offline verifier for it** — worth nothing against a sound CSPRNG, and the difference between *exploitable by an attacker present at the time* and *exploitable by anyone who recorded it, at any later date* against a weak one. This is 7.2a's point arriving on the new path, and the real force of 11.5a's MUST. |
| **E39** | [`PPCP-RV` 9e1](ppcp-rv.md#9-conformance) | **Added, 24 August 2026 — finding R-05, PinPointStudio.** An implementation claiming §11 states **which of the initiator and acceptor roles** it provides, and 9e's *in full* binds each role claimed rather than requiring both. 9e was written about not skipping halves of the exchange and said nothing about roles — and one peer on this deployment ships initiator-only, so two implementations could each claim §11 and be unable to pair. |
| **E40** | [`PPCP-RV` 11.6c1, 11.6c](ppcp-rv.md#116-derivation), [RT-24a](ppcp-rv.md#9-conformance) | **Added, 24 August 2026 — finding R-09, PinPointStudio, second review pass.** E34 summarised its transcript binding as a general rule — *everything that varies is bound into everything derived* — and offered it explicitly as the thing to hold in mind **instead of** the individual formulas. It is not true of the two clauses below it: `sid` and `PRK` are functions of the shared secret alone, deliberately. An implementer trusting the rule binds the transcript into `sid` and gets **matching digits, matching MACs and a divergent `PRK`** — a successful comparison, a successful confirmation, then `PSK_IDENTITY_NOT_FOUND`, which RV §10.4 warns will be misdiagnosed as the 3.5d platform limitation. The trap did not exist before E34. The rule is scoped and the boundary stated with its reason: by the time `PRK` is derived the exchange is already authenticated, and §5.1 is taken verbatim so its inputs cannot be changed without giving §5.1 a second shape. |
| **E41** | [`PPCP-RV` 11.4h1](ppcp-rv.md#114-frames) | **Added, 24 August 2026 — finding R-10, PinPointStudio.** The bootstrap version `v` is **1..255** — 11.4b made it a CBOR uint reaching 2⁶⁴−1 while 11.6c encodes it into the transcript as one octet — an acceptor **echoes** the `v` it received rather than substituting one, and the document now states **which `v` each peer binds**, which was a normative derivation input left unsaid. Every consistent reading detects the downgrade E34 closed, so no hole closes; but §10.4 carries one value of `v`, so the vector agrees with every reading and RT-18 would pass two implementations that had chosen differently. An unstated derivation input the vector cannot expose is the precise shape of an interoperability failure. |
| **E42** | [`PPCP-RV` §11.7](ppcp-rv.md#117-the-short-authentication-string), [§10.4](ppcp-rv.md#104-guided-pairing) | **Corrected, 24 August 2026 — findings R-07, R-08 (PinPointStudio) and F-R9-3 (PinPointCapture). Prose only; no clause, vector or security claim changes.** Two arithmetic errors, both inside notes whose purpose is to stop a reader making an arithmetic error. The modulo-bias paragraph claimed `295 967 296` residues where only 10⁶ exist, put the favoured set the wrong way round, and understated the excess over uniform by **33×**. §10.4's little-endian example was mis-reversed at source **and** computed from the pre-E34 value, three lines below the box warning against exactly that. Two of the three figures came from PinPointStudio's own first-pass review and were quoted onward in good faith. Both teams recomputed every row of the vector, twice, and neither recomputed the numbers in the prose beside them. **Attribution corrected at the third pass: the wrong count came from PinPointCapture's review, not PinPointStudio's — whose figure was right — and PinPointCapture corrected the record against itself.** |
| **E43** | [`PPCP-RV` 11.6c2, 11.6c1](ppcp-rv.md#116-derivation), [§10.4](ppcp-rv.md#104-guided-pairing) | **Amended, 24 August 2026 — finding R-11, PinPointStudio, third review pass; independently raised by PinPointCapture.** E40's rationale claimed *"`Z` already commits to both public keys by construction"*. **X25519 is not contributory** — clamping makes every scalar a multiple of 8, so a counterpart can present a **different** public key yielding a **bit-identical, non-zero** shared secret; 11.6b does not fire, and OpenSSL 3.6.3 accepts the substituted key without complaint. Under that substitution `BK`, `sid` and `PRK` are all identical and **only the explicit `pk_i \|\| pk_a` in `sas_raw` separates the peers** — the binding the sentence called redundant. A reader believing it had been handed the argument for deleting the one clause holding the property up. Claim deleted, 11.6c2 forbids the simplification, §10.4 carries the witness. Not an attack; no new check wanted. **The third pass running in which a defect arrived through a rationale rather than a clause.** |
| **E44** | [`PPCP-RV` §10.4](ppcp-rv.md#104-guided-pairing), [RT-24a, RT-24b](ppcp-rv.md#9-conformance) | **Amended, 24 August 2026 — findings R-12, R-13 (PinPointStudio) and F-R9-4 (PinPointCapture).** §10.4 named its sixth divergence cause as the **only** one producing a successful-looking pairing; **cause 1 has the identical signature**, since `sid` feeds only `PRK` — so in the paragraph whose job is triaging `PSK_IDENTITY_NOT_FOUND` the reader was routed away from the older and more familiar cause. Counter-vectors added — what a **wrong** implementation produces — making a reproduction self-checking. RT-24a becomes `review`: as `static` it asserted nothing RT-18 did not. |
| **E45** | [`PPCP-RV` 11.9d1](ppcp-rv.md#119-aborting-and-the-one-attempt-rule) | **Added, 24 August 2026 — finding R-14, PinPointStudio.** A peer aborting `unsupported_version` offers the pairing code on the **first** abort: 11.4h guarantees a retry fails identically, so 11.9d's two-abort threshold spent an operator attempt on a certainty. Bounds RV-6's one version gap — a **newer initiator cannot pair with an older acceptor**, which on this deployment is the normal ship order — to *pairs by code, this once*. Negotiation deliberately not added; recorded at RV B18 with both teams' reasoning and the constraint on any future answer. |
| **E46** | [`PPCP-RV` 11.4c1](ppcp-rv.md#114-frames) | **Added, 24 August 2026 — finding R-15, PinPointStudio.** A peer **rejects an unrecognised map key** in a bootstrap frame — the one closed vocabulary in the protocol set, against `RV` 3.3a, 4.2c and A4 all pointing the other way. Two implementations could have differed with neither wrong and no vector could expose it: the **third** instance in three passes of that class. |
| **E47** | [`PPCP-RV` 3.4d3](ppcp-rv.md#34-resolvable-identifiers) | **Added, 24 August 2026.** A multi-pairing **advertiser** whose counterparts cannot advertise (3.5d) rotates far faster than 3.4a's 15-minute floor, which bounds *frequency* and not the reverse. 3.4d1 makes the reconnection wait *rotation × pairings held*, and **E27's mitigation for it assumes the multi-pairing peer is the one that can browse** — this deployment inverts that, so the host accumulates pairings, must advertise, and gains nothing by browsing. Two and a half hours at the floor with ten pairings; under four minutes at twenty seconds. Raised once `PPCP-RV` question 3 was answered, which made the case live. |
| **E48** | [`PPCP-RV` §11.11](ppcp-rv.md#1111-where-x25519-comes-from), [RT-27](ppcp-rv.md#9-conformance) | **Added, 24 August 2026 — closing `RV` B17, raised by PinPointStudio.** **Where X25519 comes from**, stated **in octets rather than as an interface**: exactly two 32-octet values — a peer's own public key and the shared secret — cross the boundary, and **nothing else does**, because it sits *below* the derivation rather than inside it. It is a **parameter, not a callback**: key agreement has no loop, so the component holding the private scalar computes the secret once and hands it over exactly as it already hands over `psk`, `sid` and `rn`. §11's one primitive this protocol set has never needed is thereby kept out of the implementation that carries no cryptographic library by construction. **11.11f is the clause with teeth**: E36's two measured behaviours split across the boundary — one side sees only a failed call, the other only an all-zero output — so both map to `invalid_key`, neither to a transport error, and neither is retried. The gain beyond tidiness is 11.11c: the derivation becomes a pure function of the shared secret and the transcript, so `RV` §10.4 is reproducible and RT-18 runnable by a component with no curve arithmetic — in the one component both applications share, rather than twice in two that would then agree with themselves (`RV` B7). |
| **E49** | [`PPCP-RV` 3.2d](ppcp-rv.md#32-instance-name), [3.4d3](ppcp-rv.md#34-resolvable-identifiers) | **Added and corrected, 24 August 2026 — finding R-19, PinPointStudio.** E47 required a multi-pairing advertiser to rotate far faster than the 15-minute floor and described the cost as *"only multicast chatter"* and the effect as *"strictly better for unlinkability"*. **Both were wrong.** 3.2a ties the instance name to `rid`, so every rotation **renames the service** — deregister, probe, announce, not a TXT update — and at seconds-scale that is continuous traffic on exactly the networks 3.6a says rate-limit and drop multicast: **the mitigation for a discovery problem would have triggered the condition that breaks discovery.** And within one registration the address and port are constant, so an observer links the rotations by inspection — faster rotation is **neutral** for unlinkability, 3.4e's cross-venue property being what rotation actually buys. 3.2d is the fix: a stable per-registration random instance name, rotating only the TXT, one announcement instead of a cycle. Browsers are unaffected — 3.4b resolves `rid` from the TXT, never from the name. |
| **E50** | [`PPCP-RV` RT-20a/b/c, RT-24b/c](ppcp-rv.md#9-conformance) | **Amended, 24 August 2026 — finding R-20, PinPointStudio, and the standing report from both teams that RT-20 had not moved in four passes.** RT-20 was **one row bundling three claims of different kinds** — a computation, a per-peer behaviour and an interop result — so the whole waited on the last of the three and was reported unrun for the whole of development while two thirds were runnable far earlier. **A defect in the test, not in the protocol.** RT-20a is arithmetic over four keypairs, runs before any of `RV` §11 exists (11.11c having made the derivation pure), and a large run **measures** §11.8's 2⁻²⁰ bound rather than restating it. RT-20b needs **one** implementation plus the relay and covers 11.5c's ordering — the load-bearing clause — which a relay observes directly by withholding `bs_reveal`. RT-20c is the interop claim and the only part needing both. RT-24b splits likewise: its cofactor witness needs an actual X25519 call the deriving component may be unable to make. **RT-20c remains the conformance claim; a and b passing is not it.** |
| **E51** | [`PPCP-RV` 11.6f](ppcp-rv.md#116-derivation), [11.11h1](ppcp-rv.md#1111-where-x25519-comes-from) | **Added, 24 August 2026 — finding F-R9-5, PinPointCapture.** 11.6f's prose carried the failure case — *"what survives a failed one is nothing"* — and its **explicit list did not**, naming four items of which `PRK` was not one. A list is what gets implemented from, and the exposure is real: a peer computes the whole chain the moment it holds the shared secret, with 11.3e allowing sixty seconds for a user to affirm, so **every abort path left a `PRK` in memory for a pairing that does not exist and never will**. The list now names `PRK`, `K_tls`, `K_id` and `sid` on the failure path. 11.11h1 records that on at least one platform the erasure obligation falls **partly on a closed-source framework** that neither exposes a zeroise nor documents one — the same class as `RV` §5.4's platform limits: not a defect, a bounded truth about what a MUST can mean. |
| **E52** | [`PPCP-RV` 9g](ppcp-rv.md#9-conformance) | **Added, 24 August 2026 — PinPointStudio, fourth pass.** A conformance claim to `RV` §11 **names RT-20c and states its result**, and **no aggregate pass** may be reported while it is unrun. **Every other test in that list checks that two *honest* implementations compute the same numbers; RT-20 is the only one in which somebody is attacking.** The hazard is the volume of the rest — four vector reproductions, two independent implementations, four review passes, twenty findings closed, and none of it touching the security property. `RV` 11.1d already names the extreme case: a peer that quietly compares the digits itself passes every static test in the document while authenticating nothing. Splitting RT-20 under E50 added *more* green and would have made this worse without the guard. The clause also records what each part does **not** prove — RT-20c shows the protocol emits a mismatch signal, **not** that a tired operator notices it — and fixes the build order: the relay is the **first** artefact, not the last, because both security-touching tests depend on it and it needs no application to exist. |
| **E53** | [`PPCP-RV` §11](ppcp-rv.md#11-rv-6--guided-pairing) preamble, [3.7h](ppcp-rv.md#37-the-bootstrap-window), [3.4d3](ppcp-rv.md#34-resolvable-identifiers), B15 | **Premise corrected, 24 August 2026 — protocol owner. Non-normative throughout; no clause changes.** [CR-01](../changerequests/CR-01-in-band-pairing.md) was granted for *"a venue where a range operator sets up several bays"* and **that deployment does not exist** — the host is never at the range. A capture device there works standalone and its session travels home as a bundle (§9, `ENC` §7); the two peers meet on a **home or coaching-studio network**, which the user controls and where multicast behaves. So `RV` 3.6a's *"it will not work at a range"* never bounded this feature, 3.7h is an ordinary MAY rather than the clause deciding whether RV-6 was worth building, B15's fleet case is a studio's two or three stereo devices, and 3.4d3's rotation is sized for a handful of pairings. **RV-6 is kept on a narrower basis than it was granted on**: comparing six digits needs no camera, no focus and no line of sight, which beats scanning a code even for one phone and one PC in one room. ⛔ **Four review passes and twenty findings never asked whether the deployment existed** — both teams inherited the premise from the request, and so did the owner. |
| **E54** | [`PPCP-RV` 7.1](ppcp-rv.md#71-threat-model), [§11.8](ppcp-rv.md#118-what-the-comparison-proves), [§10.4](ppcp-rv.md#104-guided-pairing), [RT-20a/b](ppcp-rv.md#9-conformance) | **Corrected, 24 August 2026 — R-21/R-22/R-23 (PinPointStudio), F-R9-6/F-R9-7 (PinPointCapture); the bound found independently by both and computed identically.** **The guided-pairing bound is 1 in 1 000 000, not 1 in 1 048 576.** 2⁻²⁰ is the width of `sas_raw`; `RV` 11.7a reduces it mod 10⁶ before anyone sees it and an attacker must match the **digits**. Exactly `144115188323/144115188075855872` = 1.0000000017 × 10⁻⁶, **4.86% weaker than published** — immaterial to the argument, material to a conformance row told to check a measurement against it. It also settles what E42 could not: the modulo bias contributes 1.7 × 10⁻⁹ relative and **does not reach the bound**. RT-20a becomes **deterministic** against a published **interposer quadruple** carrying both legs' shared secrets, so the one test touching the security property needs no key agreement — which 11.11c had promised and the row had not delivered; its statistical half becomes *no collision, and uniformity by χ²*, never the rate, since separating 10⁻⁶ from a 5% neighbour needs ~10⁹ trials for 1.5 σ and **a row nobody can run is a row that gets ticked**. RT-20b(ii) gains the **initiator mirror**, 9e1 permitting a peer to claim one role and one implementation here claiming each. **First execution of RT-20a: 200 000 quadruples, zero collisions, χ² 933.6/999.** |
...
*CR-02's specified text went to both implementation teams for review against the disposition's own three-round cap. **E63–E64 are round 1** — one blocking, cheaply-fixed cardinality gap found independently by both teams, and one polish item.*

| **E63** | [I39](#11-invariants), [`PPCP-MSG` 12.1c1, 12.2a1](ppcp-messages.md#121-actuator_command--actuator_command_ack) | **Amended, 26 August 2026 — CR-02 review round 1, finding F1 (PinPointCapture) and finding R-1 (PinPointStudio), raised independently by both teams against the same clause.** I39 as first specified bound only `actuator_command`'s `on`/`level` cardinality. `actuator_command_ack.state` and `actuator_state.state` carry the identical `{ on, level }` shape but each field was marked independently optional, so a conformant peer could send `state: {}` — satisfying the schema while contradicting 12.1c's own prose that `state` reports what the Actuator is actually doing. I39 now binds all three messages identically; `actuator_command_ack.state` and `actuator_state.state` carry `on` iff `control: on_off` and `level` iff `control: level`, on the same terms as the request. Text-only; no field added, nothing renumbered. |
| **E64** | [5.20d](#520-devicestatus) | **Amended, 26 August 2026 — CR-02 review round 1, finding F2, PinPointCapture.** `DeviceStatus.reason`'s example list carried `no_source` over from `Readiness.blocked_reason`'s vocabulary. `device_status` is emitted only for an already-declared Source (`PPCP-MSG` 5.5c), so the value names a case the event's own precondition rules out. Dropped from the list; 5.20d records why, so an unreachable value does not get cargo-culted into an implementation's `reason` handling. |
| **E65** | [`PPCP-MSG` 5.5a, 12.2a, §11](ppcp-messages.md#11-message-index), [`PPCP-CONF` §3](ppcp-conformance.md#3-the-invariant-test-matrix) | **Amended, 1 September 2026 — CR-02 implementation, `libppcp`.** CR-02's own text did not pass the [`PPCP-CONF` 5b1/5b2](ppcp-conformance.md#5-interoperability) audit that [E18](#errata-after-revision-9) installed, and `ctest -R L16-profile-boundary` was red on three findings the moment E58–E60 landed. **5.5a** stated its obligation as *"Emitted whenever …"* without naming `device_status`, so the audit could not see that §11's citation of it was supported — reworded to name the message, exactly as E18 reworded eight other clauses for the same reason. **§11** marked `actuator_state` **opt** although 12.2a requires a peer to emit it; the row now cites 12.2a. **12.2a** named two other messages in passing, and the audit marks every catalogued message a qualifying MUST names, so it was also read as requiring a peer to originate the actuator request — those names are now written in prose, leaving §11's correct **opt** for a message no clause requires anyone to send. §11's tally of messages required by nothing moves from fifteen to seventeen, and `PPCP-CONF` §3's header count from *"thirty-eight invariants, thirty-nine tests"* to thirty-nine and forty, which I39 had already made stale. **Text-only; no field added, no obligation added or removed, nothing renumbered.** |
| **E66** | [`PPCP-MSG` 3.3, 3.3f](ppcp-messages.md#33-declare) | **Added, 1 September 2026 — CR-02 implementation, `libppcp`.** [E58](#errata-after-revision-9) added `actuators` to the `Peer` entity ([§5.2](#52-peer)) but never amended `declare`'s schema block, which lists five fields and does not name it. Both readings were defensible — nest it in the `peer` head, since `Peer` is where the field lives and the block's own note excludes only `sources`; or hoist it beside `sources`, since that note is the exception that exists *because* an entity list was hoisted. **Two conformant peers would each have declared Actuators the other could not see, with nothing malformed on the wire and no error at either end.** 3.3f pins it as a top-level sibling of `sources`, omitted entirely where a peer owns none (5.19c) — so a pre-erratum declaration stays byte-identical and no fixture moves. Found by the implementation having to choose, which is how [E28](#errata-after-revision-9) was found too. |
| **E67** | [5.4d](#54-timebaserelation) | **Added, 2 September 2026 — `libppcp`, PinPointStudio.** An `affine` relation may be read from either end, and a conversion resolves to the declared relation or the inverse of the reverse one, whichever has the smaller sigma at the instant. Each of two peers estimates the same pair of clocks and publishes its own relation, keyed by direction — so a host converting a device's instant was bound to the **device's** estimate, and waited on it: measured over WiFi, the host's own estimate of the same relation was under a 5 ms policy 1–4 s after connect on every link while the device's took 7 s idle and 32 s or longer when the device was busy. Inversion is exact and is not composition (5.4c, I18); `unrelated` in either direction still wins (IOP-5). |
| **E68** | [6.3c1, 6.3g1](#63-clock-synchronisation) | **Added, 2 September 2026 — `libppcp`.** The connect burst is spread over one to three seconds and the first maintenance exchanges follow at 0.5, 1, 2 and 3 s. A burst of sixteen exchanges 20 ms apart spanned 320 ms, over which the fitted rate was noise (skew sigma ~600,000 ppm, residual 25–40 ms, measured on every connect), so the relation was unusable until the first five-second maintenance exchange — a floor of 5–7 s on every link even when everything was clean. Steady cadence unchanged: raising it was measured to make things worse on a shared radio ([6.3g1](#63-clock-synchronisation)). |
| **E69** | [6.3f1](#63-clock-synchronisation) | **Added, 2 September 2026 — `libppcp`.** The minimum-RTT filter admits exchanges relative to the lowest RTT seen rather than as the best half by count, and a bounded window retires by quality (highest RTT first, with an age bound), not by arrival order. Best-half-by-count over an oldest-first window kept a noisy connect burst in the fit until sixteen clean maintenance exchanges had displaced it — 80 s at one per five seconds — which was the mechanism behind the 72–120 s convergence measured on 29 August 2026, not link quality and not cadence. |
