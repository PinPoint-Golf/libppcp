...
| Revision | 9 — adds RV-6 (guided pairing), plus errata E34–E55 from **five** review passes over it. **CR-01 is closed: no open specification items.** Revision 8 was final; [CR-01](../changerequests/CR-01-in-band-pairing.md) reopened it. ⚠ **[E56](#errata-after-revision-9--change-request-cr-01-and-its-review) is the first erratum after CR-01 closed** and is unrelated to it: [7.2c](#72-handling-the-pairing-secret) MUST → SHOULD. |
...
Both are reduced by encrypting the volume, which costs a deployment nothing on any current platform and is the mitigation to state in an operator-facing note. Neither is removed by it: an attacker running **as the user** on an unlocked machine reads the file through the encryption. What remains after that is [7.4b](#74-persistent-pairings) — persistence that is opt-in, visible and individually revocable — which is why [E56](#errata-after-revision-9--change-request-cr-01-and-its-review) leaves it a MUST while relaxing the clause beside it.
...
⚠ **The honest reading of what this gives up.** A `PRK` in an ordinary settings file is readable by anything running as that user. [7.4a](#74-persistent-pairings) was always a MAY and is now a weaker one, and [7.4b](#74-persistent-pairings)'s opt-in, visible, individually revocable persistence is the control that survives — it was the weaker-looking half of the pair and it is now the load-bearing one. **No compensating MUST was added**, deliberately: every candidate for one was another storage decision, which is the thing this erratum exists to stop making.
