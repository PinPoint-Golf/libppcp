...
| Status | **APPROVED for implementation**, 22 August 2026. **Revision 9 adds [RV-6](#11-rv-6--guided-pairing) under [CR-01](../changerequests/CR-01-in-band-pairing.md); both teams reviewed it three times and both return *ready to implement*.** Fifteen findings over three passes, applied as errata E34–E47, plus E48 closing [B17](#annex-b--open-issues); **no open findings**. What is not yet demonstrated is [RT-20](#9-conformance), and that has not moved. |
| Date | 22 August 2026 |
...
| Revision | 9 — adds RV-6 (guided pairing), plus errata E34–E48 from three review passes over it and the closure of B17. Revision 8 was final; [CR-01](../changerequests/CR-01-in-band-pairing.md) reopened it. |
| Change requests | [`../changerequests/`](../changerequests/) — [CR-01](../changerequests/CR-01-in-band-pairing.md), its [disposition](../changerequests/CR-01-disposition.md), and the responses to the [first](../changerequests/CR-01-review-response.md), [second](../changerequests/CR-01-review-response-2.md) and [third](../changerequests/CR-01-review-response-3.md) review passes |
...
- **(3.4d3) SHOULD** *Erratum E47, 24 August 2026.* A peer that advertises for reconnection, holds **several** pairings, and whose counterparts **cannot advertise** ([3.5d](#35-who-advertises-and-who-browses)) rotates **substantially faster than the 15-minute floor** of [3.4a](#34-resolvable-identifiers) — seconds rather than minutes — so that the wait of [3.4d1](#34-resolvable-identifiers) stays tolerable. [3.4a](#34-resolvable-identifiers) sets a floor on *frequency*, not a ceiling: rotating more often is conformant, costs only multicast chatter, and is strictly **better** for unlinkability.
  ...
  At the 15-minute floor with ten pairings that is a wait of up to two and a half hours before a device sees an advertisement it can resolve. At twenty-second rotation it is under four minutes. **The clause costs nothing and the alternative is a reconnection feature that appears not to work**, which is the outcome [§3.6](#36-multicast-is-not-to-be-relied-on)'s fallback would then mask as a missing code prompt.
...
| **RT-24b** | static | The **counter-vectors** of [§10.4](#104-guided-pairing) reproduce: the two wrong `PRK`/`sid` values a mis-derivation produces, and the R-11 witness — a substituted `pk_a'` giving a **bit-identical non-zero `Z`** and different digits (11.6c2). Distinct from [RT-18](#9-conformance) because it asserts what a **wrong** implementation yields, which is what makes a reproduction self-checking rather than merely matching; and the witness is the one an implementer should run before concluding the explicit key binding is redundant. |
