...
- **(3.4d3) SHOULD** *Erratum E47, 24 August 2026.* A peer that advertises for reconnection, holds **several** pairings, and whose counterparts **cannot advertise** ([3.5d](#35-who-advertises-and-who-browses)) rotates **substantially faster than the 15-minute floor** of [3.4a](#34-resolvable-identifiers) — seconds rather than minutes — so that the wait of [3.4d1](#34-resolvable-identifiers) stays tolerable. [3.4a](#34-resolvable-identifiers) sets a floor on *frequency*, not a ceiling: rotating more often is conformant, costs only multicast chatter, and is strictly **better** for unlinkability.
  ...
  At the 15-minute floor with ten pairings that is a wait of up to two and a half hours before a device sees an advertisement it can resolve. At twenty-second rotation it is under four minutes. The alternative is a reconnection feature that appears not to work, which is the outcome [§3.6](#36-multicast-is-not-to-be-relied-on)'s fallback would then mask as a missing code prompt.
...
⛔ **Conformant and *solves the problem* are different states here, and both teams asked that this not stay in an annex.** CR-01 was raised for a driving range. [3.6a](#36-multicast-is-not-to-be-relied-on) says of multicast, without hedging, that **it will not work at a range** — rate-limited by consumer access points, blocked by client isolation, and it does not cross VLANs. So **RV-6 reached over mDNS is a feature that works in an office and not at the venue it was requested for.**

The clause that matters there is [3.7h](#37-the-bootstrap-window) — a bootstrap window reached at an endpoint learned **out of band** — which is a MAY, which neither application implements today, and about which this document correctly says nothing further, because [§11](#11-rv-6--guided-pairing) constrains the handshake and not how the endpoint was found. That is not a defect and no clause change was sought by either reviewer. It does mean the implementation work item is *how a window is reached without multicast*, **not** *how a window is dialled* — and that a demonstration succeeding in an office and failing at a range is the worst available place to discover it.
