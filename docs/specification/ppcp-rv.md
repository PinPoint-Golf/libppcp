...
| Status | **APPROVED for implementation**, 22 August 2026. **Revision 9 adds [RV-6](#11-rv-6--guided-pairing) under [CR-01](../changerequests/CR-01-in-band-pairing.md); both teams reviewed it three times and both return *ready to implement*.** **Twenty-six findings over five review passes**, applied as errata E34–E55, including [E53](#errata-after-revision-9--change-request-cr-01-and-its-review) correcting the deployment premise the request was granted on; **no open findings**, and both teams have no objection to implementation starting. ⛔ **[RT-20c](#9-conformance) is unrun and [9g](#9-conformance) forbids reporting an aggregate pass without it.** What is not yet demonstrated is [RT-20](#9-conformance), and that has not moved. |
| Date | 22 August 2026 |
...
| Revision | 9 — adds RV-6 (guided pairing), plus errata E34–E55 from **five** review passes over it. **CR-01 is closed: no open specification items.** Revision 8 was final; [CR-01](../changerequests/CR-01-in-band-pairing.md) reopened it. ⚠ **[E56](#errata-after-revision-9--change-request-cr-01-and-its-review) and [E57](#errata-after-revision-9--change-request-cr-01-and-its-review) are the first errata after CR-01 closed** and are unrelated to it: [7.2c](#72-handling-the-pairing-secret) and [7.4b](#74-persistent-pairings) MUST → SHOULD, the specification withdrawing from decisions that belong to the application. |
...
**A third pass, over the second's fixes, found [E43](#errata-after-revision-9--change-request-cr-01-and-its-review)** — a rationale asserting a property X25519 does not have, which licensed deleting the one binding that holds the whole construction up.

**[§4.3b](#43-payload) is why this document has vectors; E34 and E35 are why it has review passes; E40 and E43 are why the fixes get reviewed too — and E37, E40 and E43 together are why the *explanations* need reviewing as carefully as the clauses**, because they are what an implementer reads first and they carry more authority than their authors intend.
...
- **(3.2d) MAY** *Erratum E49, 24 August 2026 — finding R-19, PinPointStudio.* A peer rotating `rn` **more often than [3.4a](#34-resolvable-identifiers)'s floor** — which [3.4d3](#34-resolvable-identifiers) requires of a multi-pairing advertiser — keep a **stable instance name for the duration of one registration**: `PPCP-` followed by eight uppercase hexadecimal characters of four CSPRNG bytes, drawn fresh for **each registration**, and rotate only the TXT record. [3.2b](#32-instance-name) binds it unchanged and is satisfied by construction — a value drawn per registration persists across nothing.

  ⛔ **Without this, [3.4d3](#34-resolvable-identifiers) would be self-defeating.** [3.2a](#32-instance-name) derives the instance name from `rid`, so rotating `rid` **renames the service** — a deregister, probe and announce cycle, not a TXT update. At the fifteen-minute floor that is negligible. At the seconds-scale rotation 3.4d3 calls for it is continuous, on **precisely the networks [3.6a](#36-multicast-is-not-to-be-relied-on) says rate-limit and drop multicast**: the mitigation for a discovery problem would trigger the condition that breaks discovery.

  A browser is unaffected either way. [3.4b](#34-resolvable-identifiers) resolves `rid` from the **TXT record**, never from the instance name, so 3.2a's derivation is a convention rather than a mechanism and nothing reads it.
...
- **(3.4d3) SHOULD** *Erratum E47, 24 August 2026.* A peer that advertises for reconnection, holds **several** pairings, and whose counterparts **cannot advertise** ([3.5d](#35-who-advertises-and-who-browses)) rotates **substantially faster than the 15-minute floor** of [3.4a](#34-resolvable-identifiers) — seconds rather than minutes — so that the wait of [3.4d1](#34-resolvable-identifiers) stays tolerable. [3.4a](#34-resolvable-identifiers) sets a floor on *frequency*, not a ceiling: rotating more often is conformant, costs only multicast chatter, and is strictly **better** for unlinkability.

  [3.4d1](#34-resolvable-identifiers) bounds the wait to *the rotation period times the number of pairings held*, and [3.4d2](#34-resolvable-identifiers) mitigates it by having a multi-pairing peer **browse as well as advertise** — resolving every held pairing in one pass with no rotation and no waiting. ⛔ **That mitigation assumes the multi-pairing peer is the one that can browse, and this deployment inverts it.** The host is the peer that accumulates pairings — every capture device that has used that bay — *and* it is the peer that must advertise, because [3.5d](#35-who-advertises-and-who-browses) forbids its counterparts from doing so. Browsing gains it nothing: there is nothing to find. So the one mitigation [E27](#errata-after-revision-8) provided is precisely the one unavailable, leaving only 3.4d1's *recently-used-first* SHOULD.

  ⛔ **Size the rotation on pairings *held*, not on devices *present*** — *erratum E55, 24 August 2026, PinPointStudio, carrying [E53](#errata-after-revision-9--change-request-cr-01-and-its-review) into this clause.* The paragraph above argued from a venue where devices move between bays, and [E53](#errata-after-revision-9--change-request-cr-01-and-its-review) established that **that deployment does not exist** — the host is not at the range. The clause survives it, and the reason is sharper than the one first given: **[7.4a](#74-persistent-pairings) gives a persisted pairing no expiry.** It ends on revocation ([7.4d](#74-persistent-pairings)) and on nothing else. So a coaching-studio host **accumulates pairings indefinitely** — every device it has ever paired with, for as long as nobody prunes them — and the count that sets the wait is that accumulated total, not the two or three devices in the room today. [7.4b](#74-persistent-pairings)'s visible, individually revocable persistence is the only pruning mechanism there is, it is a user action rather than an automatic one, and since [E57](#errata-after-revision-9--change-request-cr-01-and-its-review) it is a SHOULD — so a peer that offers no pruning at all accumulates without bound and is still conformant.

  At the 15-minute floor with ten pairings that is a wait of up to two and a half hours before a device sees an advertisement it can resolve; at three pairings, three quarters of an hour. At twenty-second rotation both are minutes. The alternative is a reconnection feature that appears not to work, which is the outcome [§3.6](#36-multicast-is-not-to-be-relied-on)'s fallback would then mask as a missing code prompt.

  ⚠ *Recalibrated by erratum E53.* The ten-pairing figure came from CR-01's several-bays venue, **which is not this deployment** — the host is not at the range ([§11](#11-rv-6--guided-pairing)'s preamble). A home or coaching-studio host holds **few** pairings: one or two phones, or a handful of a coach's pupils' devices. **The clause is unchanged and still wanted** — it binds any multi-pairing advertiser whose counterparts cannot advertise, which is exactly what a studio host is — but its urgency is a studio's three devices rather than a venue's twenty, and an implementation should size the rotation to what it actually holds rather than to the figure this paragraph first quoted.

  *Erratum E49, 24 August 2026 — finding R-19, PinPointStudio. Two claims in this clause were wrong as first written and are corrected here; the clause stands.*

  ⛔ **It is not "only multicast chatter".** [3.2a](#32-instance-name) ties the instance name to `rid`, so each rotation **renames the service instance** — a full deregister/probe/announce cycle rather than a TXT update — and at seconds-scale that is continuous traffic on the very networks [3.6a](#36-multicast-is-not-to-be-relied-on) describes as dropping it. That is why [3.2d](#32-instance-name) exists: with a stable per-registration name the rotation is **one announcement**, which is what makes this clause affordable.

  ⚠ **And it is *not* "strictly better for unlinkability".** Within one registration the `A`/`AAAA` record and the port do not change, so an on-link observer links every rotation trivially — they are the same host by inspection. Rotation's value is the one [3.4e](#34-resolvable-identifiers) actually claims, that *observations in two venues cannot be correlated*, and that is unaffected by rotating faster inside one venue. Faster rotation is **neutral** here, and this clause should not lean on a benefit it does not have.

  ✅ **Checked and sound, recorded so it is not raised as the obvious objection:** fast rotation does **not** leak the count of held pairings. [3.4a](#34-resolvable-identifiers) forces rotation regardless, so an observer sees churning `rid`s either way and cannot distinguish one pairing from ten. [B3](#annex-b--open-issues)'s property survives E47 intact.
...
- **(3.5e) SHOULD** *Erratum E32, 24 August 2026 — CR-01.* Where a peer's counterpart **cannot** advertise for reconnection under [3.5d](#35-who-advertises-and-who-browses), the peer that **can** advertises. This is the reversal of [3.5c](#35-who-advertises-and-who-browses) stated as an obligation rather than a permission, and it is here because 3.5d only says who must *not* advertise: read together with [3.5b](#35-who-advertises-and-who-browses)'s recommendation that the capture peer does, a deployment could conclude that **neither** end advertises and satisfy every clause while doing so. In that deployment [§7.4](#74-persistent-pairings)'s persisted pairing buys nothing at all — both peers hold valid key material and no path exists by which either finds the other — and the users see a protocol that remembers them and still asks for a code every session.
  ...
  CR-01 §9 question 3 asks whether the host will advertise. 3.5e is the answer this document can give: on a deployment whose capture peer is bound by 3.5d, **the host advertising is what makes persistence work**, and it is a SHOULD rather than a MUST only because [§3](#3-rv-1--service-discovery) as a whole is optional and a host reachable at a cached endpoint has another way. 3.5c is still right that supplying a responder is a platform question — but it is no longer a question the specification leaves open on which peer *should*.

  ✅ **CR-01 question 3 is answered. The protocol owner has committed the deployment to this shape, 24 August 2026**, and the reasoning is worth recording because it is stronger than *"3.5d leaves no choice"*: **host-advertises / device-dials is the better pattern on its own merits, not merely the permitted one.** A foregrounded application can dial reliably; a backgrounded one cannot reliably *listen*, and capture requires the foreground anyway ([3.5b](#35-who-advertises-and-who-browses)'s own reasoning). It also matches the workflow — the host is set up first and is already advertising, and the capture device joins when it comes online, rather than the host hunting for a device that may not be listening yet. The platform limitation of [3.5d](#35-who-advertises-and-who-browses) and the ergonomics point the same way, which is the comfortable case and is rarer than it should be. See [A18](#annex-a--decisions-and-alternatives).
...
- **(3.7h) MAY** A guided pairing be reached **without** discovery, at an endpoint entered or configured out of band. [§11](#11-rv-6--guided-pairing) constrains the handshake and not how the endpoint was learned.

  *Amended by erratum E53, 24 August 2026.* This clause read that [3.6a](#36-multicast-is-not-to-be-relied-on) *"applies here with more force than anywhere else in this document — multicast is least reliable at exactly the venue this path was asked for"*, and on that reading both implementation teams raised 3.7h as the clause deciding whether RV-6 was worth building. **The premise was wrong**: the host is not at the range, so the two peers meet on a home or studio network rather than a venue one ([§11](#11-rv-6--guided-pairing)'s preamble). 3.7h is an ordinary MAY for the ordinary case — a cached endpoint, a tunnel, a directly-entered address — and carries no special weight.
...
- **(5.4h) SHOULD** A peer erase a session's derived key material at session close unless the pairing is persisted ([7.2d](#72-handling-the-pairing-secret)), and a persisted pairing remains visible and revocable where [7.4b](#74-persistent-pairings) is honoured — a SHOULD since [E57](#errata-after-revision-9--change-request-cr-01-and-its-review). Key material that no longer exists cannot be disclosed later.
...
| **A one-in-a-million guess by a man-in-the-middle on a guided pairing** | Bounded, not eliminated. [§11.8](#118-what-the-comparison-proves) states the number and what keeps the attacker to one draw of it. A successful guess is an undetected MITM for the life of the pairing, which is why [7.4b](#74-persistent-pairings)'s revocability matters as much on this path as on any — ⛔ and since [E57](#errata-after-revision-9--change-request-cr-01-and-its-review) made 7.4b a SHOULD, a peer offering no way to revoke leaves such a pairing running for as long as it exists ([7.4a](#74-persistent-pairings) gives it no expiry). |
...
- **(7.2c) SHOULD** *Erratum E56, 25 August 2026 — both implementations.* Secrets at rest are held in the platform's protected storage where one exists.
...
⛔ **7.2c was a MUST until erratum E56 and should never have been one.** [§1.3](#13-where-it-stops) lists *"platform permission handling"* and *"how a peer stores its own secrets at rest"* among the things this document does not decide, and 7.2c then reached into exactly that and decided it. Protected storage is not a free property an implementation either has or lacks: on most platforms it is a **user-facing permission**, and a specification has no standing to spend a consuming application's permission budget on its behalf. The application owns that decision because the application, not this document, faces the user who declines.

**What it cost, and it was not hypothetical.** Both implementations reported the same clause breaking [§7.4](#74-persistent-pairings) and [§7.5](#75-reconnecting-within-a-session) from opposite directions. One ships a keychain on **one** of its three platforms and — reading *"where one exists"* honestly — offered **no persistence at all** on the other two, so a paired peer was forgotten on every launch and reconnection had nothing to resolve. The other holds its secrets in a class that is **unreadable while the device is locked**, so the reconnection sweep could not read what it had stored and reported *no pairings held* — a sentence that is false and indistinguishable from the true one. **This is [E3](#errata-after-revision-9--change-request-cr-01-and-its-review)'s shape again**: a clause read literally and correctly, making a section of this document unreachable in the deployment it was written for, and found only when somebody implemented it.

⚠ **What is given up is real and is not being assumed away.** A pairing root key in ordinary application settings is readable by anything running as that user, and by anything that can read a backup of it. [7.4a](#74-persistent-pairings)'s persistence is therefore a weaker offer than it was when this clause was a MUST, and the closing paragraph of [§7.4](#74-persistent-pairings) — *possession of the device's storage is possession of continuing access* — now carries the weight that 7.2c used to share. [7.4b](#74-persistent-pairings)'s opt-in, visible, individually revocable persistence was what remained — and [E57](#errata-after-revision-9--change-request-cr-01-and-its-review) has since made that a SHOULD as well, so it is now an expectation rather than a requirement. ⛔ **Read the two errata together: after E57 nothing normative protects a persisted `PRK`.** **SHOULD is not permission to stop thinking about this** — where a platform offers protected storage at a cost the application judges acceptable, it is still the right place, and an implementation that declines it is choosing, not defaulting.
...
- **(7.4b) SHOULD** *Erratum E57, 25 August 2026 — the protocol owner.* Persistence is opt-in, visible to the user, and individually revocable.
...
**The exposure of a persisted pairing is real and should be weighed rather than assumed away**: possession of the device's storage is possession of continuing access. That is why 7.4b asks for it to be visible and revocable, and why it is optional rather than automatic.

⛔ **7.4b was a MUST until erratum E57, and the objection is to *opt-in* specifically.** *25 August 2026, the protocol owner.* **A user who is deliberately pairing a device has already said what they want.** Requiring a second, separate agreement before the pairing may be remembered puts a consent step inside a flow whose entire purpose was consent — the user scanned a code, or compared six digits and affirmed them, and is then asked whether they meant it. That is **friction bought with nothing**: it does not inform a decision the user has not already made, and the predictable result is the one both implementations hit — a default-off toggle on one screen out of two, and a paired device that is silently forgotten at the end of the session.

**And the general point, which is the one that decides it:** *it is overstepping for the protocol to tell an application how to behave.* Opt-in, visible and individually revocable is a statement about **screens**, and [§1.3](#13-where-it-stops) already lists *user interface* among the things this document does not decide. Nothing in 7.4b is observable between peers — two implementations, one offering a revocation list and one offering none, produce **byte-identical** traffic and interoperate perfectly — and the reference library cannot implement a line of it. A clause no counterpart can detect and no library can carry is a product requirement wearing a protocol's clothes. This is [E56](#errata-after-revision-9--change-request-cr-01-and-its-review)'s reasoning arriving one clause along: **the application owns the decisions whose cost lands entirely on the application.**

**The clause is kept rather than deleted, and as a SHOULD it is still the right advice** — a peer that persists silently, or that cannot show what it holds, is a worse product than one that does. What changes is who is entitled to insist.

⚠ **What this costs, stated plainly, because [E56](#errata-after-revision-9--change-request-cr-01-and-its-review) leaned on 7.4b by name.** E56 relaxed [7.2c](#72-handling-the-pairing-secret) partly on the reasoning that 7.4b remained a MUST and therefore *carried the weight 7.2c used to share*. That is no longer true, and the two errata should be read together rather than as unrelated relaxations: **after E57 there is no normative requirement protecting a persisted `PRK` at all** — not where it is stored, and not that a user can see or remove it.

**The sharp edge, and it is worth naming because it is easy to miss.** [7.4d](#74-persistent-pairings) is **unchanged and still a MUST**: revocation, *when it happens*, is honoured immediately and results in a failed handshake for the other side. What E57 removes is any requirement that a means of revoking **exists**. A conformant peer may therefore persist a pairing that its user can neither see nor end — and it is [7.4a](#74-persistent-pairings)'s lack of any expiry that makes that permanent. What still binds is narrow and worth knowing: [7.4c](#74-persistent-pairings) scopes a pairing to its counterpart, [7.4f](#74-persistent-pairings) forbids persisting one from a multi-use code, and 7.4d honours a revocation that is offered. **An implementation is expected to do the right thing here; it is simply no longer told to.**

⛔ *Erratum E56, 25 August 2026.* **This paragraph is now the whole of the argument, and it used to be half of it.** While [7.2c](#72-handling-the-pairing-secret) was a MUST, *possession of the device's storage* meant possession of a keychain — which on most platforms means an unlocked device or an authenticated user. As a SHOULD it may mean a readable file in a settings directory, and the sentence above should be read at that strength. Nothing else in this section changes: 7.4a is still a MAY, and an implementation that stores a `PRK` somewhere ordinary should be correspondingly less willing to offer persistence by default.
...
**Where a peer declines protected storage, full-disk encryption is the deployment answer.** *Added with [E56](#errata-after-revision-9--change-request-cr-01-and-its-review).* [7.2c](#72-handling-the-pairing-secret) is a SHOULD, so a `PRK` may sit in ordinary application settings, readable by anything running as that user. **For most of what that exposes the concern is bounded by an obvious argument**: an attacker who can read the file can generally read the captured sessions beside it, so the key buys them little they did not already have. Two cases are **not** covered by that argument and are worth stating rather than leaving to be re-derived:

- **The key reaches the *other* peer.** Reading a persisted `PRK` off one device confers the ability to authenticate to its counterpart, whose stored sessions were never on the compromised disk. File access on one machine becomes network access to another.
- **Recorded traffic was never at rest anywhere.** Where the peers could not reach an ephemeral exchange ([§5.2b](#52-tls-profile)) there is no forward secrecy, so a `PRK` obtained at any later date decrypts sessions captured earlier — and [7.4a](#74-persistent-pairings) gives a persisted pairing no expiry, so that window does not close on its own. Capture and key recovery need be neither simultaneous nor the same party.

Both are reduced by encrypting the volume, which costs a deployment nothing on any current platform and is the mitigation to state in an operator-facing note. Neither is removed by it: an attacker running **as the user** on an unlocked machine reads the file through the encryption. What [E56](#errata-after-revision-9--change-request-cr-01-and-its-review) pointed at as remaining was [7.4b](#74-persistent-pairings) — persistence that is opt-in, visible and individually revocable — and [E57](#errata-after-revision-9--change-request-cr-01-and-its-review) has since made **that** a SHOULD too, for the same reason: it is a statement about screens, and this document does not decide those. ⛔ **So the honest position is that nothing normative protects a persisted `PRK`** — not where it is kept, and not that a user can see or remove it. Encrypting the volume is now the *only* mitigation this section can point at, which is a reason to state it in an operator-facing note rather than to assume it.
...
- **(9g) MUST** *Erratum E52, 24 August 2026 — PinPointStudio, fourth pass.* A conformance claim to [§11](#11-rv-6--guided-pairing) **names [RT-20c](#9-conformance) explicitly and states its result**, and **MUST NOT report an aggregate pass** for RV-6 while it is unrun. [RT-20a](#9-conformance) and [RT-20b](#9-conformance) passing is not a claim that [§11.8](#118-what-the-comparison-proves)'s property holds between two implementations, and a claim that does not distinguish the three is not a claim this document recognises.

  This is [RT-11](#9-conformance)'s device — a harness that records *not applicable* rather than reporting a pass — applied where the stakes are higher, and the hazard it addresses is **the weight of accumulated green**. Every other test in this list checks that **two honest implementations compute the same numbers**: the vectors, the counter-vectors, the frame ordering, the abort codes, all of it arithmetic and bookkeeping between two parties who are both behaving. **[RT-20](#9-conformance) is the only test in which somebody is attacking**, and it is the only one that touches the property [§11](#11-rv-6--guided-pairing) exists to deliver.

  Four independent reproductions of [§10.4](#104-guided-pairing), two implementations sharing no code, four review passes and twenty findings closed is a great deal of green, and **none of it touches that property.** [11.1d](#111-what-this-path-is-and-the-one-thing-it-cannot-be) states the extreme case already: a peer that quietly compares the digits itself passes every static test in this document while authenticating nothing. **A protocol can have flawless arithmetic and no security at all**, and the gap between *specified, reviewed and undemonstrated* and *done* is invisible from the paperwork unless a clause forces it to be stated. 9g is that clause.
...
| **RT-12** | **review** | Secrets come from a platform CSPRNG at full width and are erased on revocation or session close (7.2a, 7.2d). *Amended by [E56](#errata-after-revision-9--change-request-cr-01-and-its-review):* storage is no longer part of the assertion — [7.2c](#72-handling-the-pairing-secret) is a SHOULD, so the reviewer **records where the secrets are held and whether that was a choice**, and does not fail the row for a store that is not the platform's protected one. |
...
| **RT-20a** | static | *Erratum E50, amended by [E54](#errata-after-revision-9--change-request-cr-01-and-its-review) — the interposition property as **arithmetic**.* **(a) Deterministic, and this is the required part.** [§10.4](#104-guided-pairing)'s **interposer quadruple** publishes both legs' shared secrets, so the row needs **no key agreement**: derive from the published `Z₁` and `Z₂` and assert the two legs' digits differ — `849063` against `576027`. Needs no socket, no peer, no counterpart and no curve, and runs in whichever component holds the derivation ([11.11c](#1111-where-x25519-comes-from)). **(b) SHOULD, where key agreement is available:** over a large run of random quadruples assert **no collision occurs**, and assert the displayed digits are **uniform** over 10⁶ by a χ² test — which is the property the 1-in-10⁶ bound is computed *from*. ⛔ **Do not assert the rate itself:** separating 10⁻⁶ from a 5% neighbour needs of order 10⁹ trials for 1.5 σ, so a row demanding it is a row that gets ticked rather than run. |
| **RT-20b** | injected | *Erratum E50 — the behaviour, against **one** implementation.* One real peer, the relay on the other side. Assert: **(i)** the digits it displays are the ones [11.6c](#116-derivation) gives for its own leg; **(ii)** the commitment ordering, **the clause the entire property rests on**, in whichever half the peer implements — *both mirrors, added by [E54](#errata-after-revision-9--change-request-cr-01-and-its-review), finding R-23:* an **acceptor** sends `bs_accept` **before** it has received `pk_i` ([11.5c](#115-the-exchange)), observable by a relay that withholds `bs_reveal` and checks `pk_a` already arrived; an **initiator** sends `bs_offer` carrying only `ct` and sends `pk_i` **only after** `bs_accept` arrives ([11.5b](#115-the-exchange), [11.5d](#115-the-exchange)), observable by a relay that simply does not reply and checks no `pk_i` follows. A peer claiming one role under [9e1](#9-conformance) runs its own mirror; **(iii)** a user who declines does not pair; **(iv)** the window closes and does **not** reopen without a further user action ([3.7b](#37-the-bootstrap-window), [11.9b](#119-aborting-and-the-one-attempt-rule)); **(v)** each of the relay's own legs completes on demand, or the harness is testing its own bug. |
| **RT-20c** | paired | *Erratum E50 — the **interop** claim, and the only part that needs two.* Both shipping implementations either side of the relay: both display mismatched digits, both decline, neither pairs. This is [RT-18](#9-conformance)'s agreement plus RT-20b on each side, and **nothing smaller substitutes for it.** |
...
| **RT-24a** | **review** | The implementation binds [11.6c](#116-derivation)'s transcript into **`sas_raw` and `K_c` and into nothing else** — not `ct`, not `BK`, not `sid`, not `PRK`, not either MAC label (11.6c1) — and does **not** omit `pk_i \|\| pk_a` from `sas_raw` (11.6c2). *Reworded by erratum E44 from `static`, findings R-13 (PinPointStudio) and F-R9-4 (PinPointCapture), raised independently:* as a static row it asserted nothing [RT-18](#9-conformance) did not, because a bound transcript changes `PRK` and RT-18's `PRK` row already fails — there is no state in which the value matches **and** the transcript was bound, so an implementer would re-run RT-18, tick this, and gain nothing. The assertion 11.6c1 actually wants is about the code. |
| **RT-24b** | static | The two **derivation** counter-vectors of [§10.4](#104-guided-pairing) reproduce — the wrong `PRK` of cause 1 and the wrong `sid` of cause 6, both of which produce correct digits and correct MACs and are caught nowhere else. Distinct from [RT-18](#9-conformance) because it asserts what a **wrong** implementation yields, which is what makes a reproduction self-checking rather than merely matching. Pure derivation, so it runs wherever [RT-18](#9-conformance) does ([11.11c](#1111-where-x25519-comes-from)). |
| **RT-24c** | static | The **R-11 witness** of [§10.4](#104-guided-pairing): the substituted `pk_a'` yields a **bit-identical, non-zero `Z`** and different digits (11.6c2). *Split from RT-24b by erratum E50, finding R-20, PinPointStudio* — **it needs an actual X25519 call**, which per [§11.11](#1111-where-x25519-comes-from) the deriving component may not be able to make, so it belongs on whichever side holds key agreement. Left in one row with RT-24b, a harness author finds a third of it impossible and is as likely to drop the row as to split it — and this is the block [§10.4](#104-guided-pairing) says to run *before* concluding [11.6c2](#116-derivation) is over-cautious, which makes it the one that should be hardest to skip. |
| **RT-27** | **review** | Where a boundary of [§11.11](#1111-where-x25519-comes-from) exists: **only `pk` and `Z` cross it** (11.11d); the derivation is a pure function of `Z`, `v`, `pk_i`, `pk_a` and is exercised against [§10.4](#104-guided-pairing) with the published `Z` fed in (11.11c); a **reported agreement failure and an all-zero `Z` both** produce `invalid_key` and neither is reported as a transport error (11.11f, 11.6b); and the scalar and `Z` are erased by whichever side holds them (11.11h). `review` because a boundary is invisible on the wire — an implementation that leaked `PRK` across it, or that dropped the failure signal, completes byte-identical handshakes until the day a small-order key arrives. |
...
**Three of these cannot be tested from outside**, and that is worth stating rather than leaving to be discovered. Entropy quality and erasure produce no observable difference on the wire — a peer using a predictable secret completes exactly the same handshake as one using a good secret — so **RT-12 is the requirement on which the whole model rests and the one no test can catch.** It has to be read in the code, and it should be read again whenever the key-generation path is touched. *[E56](#errata-after-revision-9--change-request-cr-01-and-its-review) narrowed what the reviewer is checking and did not make the row easier:* storage protection left the assertion because [7.2c](#72-handling-the-pairing-secret) became a SHOULD, and **the CSPRNG half is the half the model actually rests on** — the half that was always unobservable, and the one [11.11h1](#1111-where-x25519-comes-from) and [§10.4](#104-guided-pairing)'s weak-ephemeral note both arrive back at.
...
**RT-20 was one row and is now three, and that is the answer to why it had not moved.** *Erratum E50, 24 August 2026.*

As written it read *"no single-implementation harness can run it"*, and it bundled three claims of different kinds: **a computation**, **a per-peer behaviour**, and **an interop result**. Only the third needs two implementations. Because they were one row, the whole of it waited on the last of the three — and it was duly reported as *not moved* in four consecutive review passes, while two thirds of it were runnable far earlier. That is a defect in the test, not in the protocol.

| | Needs | Runnable |
|---|---|---|
| **RT-20a** | the derivation, and nothing else | **Before any of [§11](#11-rv-6--guided-pairing) is implemented.** [11.11c](#1111-where-x25519-comes-from) makes the derivation a pure function, so the interposition property is arithmetic over four keypairs — and [§10.4](#104-guided-pairing)'s published interposer quadruple makes it **deterministic**, so it needs no curve either ([E54](#errata-after-revision-9--change-request-cr-01-and-its-review)) |
| **RT-20b** | **one** implementation and the relay | As soon as either peer exists. Including [11.5c](#115-the-exchange)'s ordering, which is the load-bearing clause and which a relay observes directly by withholding `bs_reveal` |
| **RT-20c** | both, and the relay | Last, and it is genuinely last |

⛔ **What this does not do is weaken the claim.** [B7](#annex-b--open-issues) still holds: **RT-20c is the conformance claim to RV-6, and RT-20a and RT-20b passing is not it.** What changes is that a peer can now state *precisely* what it has demonstrated instead of reporting a single row as unrun for the whole of development — and that the property [§11](#11-rv-6--guided-pairing) exists to deliver gets a real, if partial, demonstration immediately rather than none for months.

**What each part proves, and what it does not.** Stated because [9g](#9-conformance) exists to stop the parts being read as the whole.

| | Proves | Does **not** prove |
|---|---|---|
| **RT-20a** | The *derivation* separates an interposer's two legs; and, where part (b) is run, that the digits are uniform over 10⁶ — which is what the bound is computed from | Anything about an implementation. It never runs against a peer, so **a peer that auto-confirms passes it trivially** ([11.1d](#111-what-this-path-is-and-the-one-thing-it-cannot-be)) |
| **RT-20b** | *This* peer displays what [11.6c](#116-derivation) says, orders its frames as [11.5c](#115-the-exchange) requires, and refuses when told to — against a relay it shares no application logic with | That the *other* implementation agrees. That is [RT-18](#9-conformance) and RT-20c |
| **RT-20c** | *These two* implementations do it against each other, with a real interposer between them | ⛔ **That the deployed thing is safe** |

⛔ **RT-20c passing shows the protocol emits a mismatch signal. It does not show that a tired operator at bay four notices it.** That half is [11.7d](#117-the-short-authentication-string) and [11.9c](#119-aborting-and-the-one-attempt-rule), it is human factors, and [§7.1](#71-threat-model)'s *not defended* table already says plainly that an operator who affirms without comparing has authenticated the attacker. **No protocol test reaches that, and none of these should be read as if it did.**

✅ **The relay is the first artefact, not the last, and that follows from the table rather than from anyone's preference.** RT-20b needs one implementation and the relay; RT-20c needs both and the relay. So the relay is a prerequisite of **both** tests that touch the security property — and it is the **earliest thing in this list that can be built**, because it needs no application at all. Building it first also produces the third implementation carrying both roles, which is the only slack the interoperable set has ([9e1](#9-conformance)'s note). **The natural gravity of a test needing two implementations is to slide to the end, where a failure costs most; the ordering here is deliberately the opposite.**
...
✅ **There is a way out, and it falls out of RT-20 itself** — PinPointStudio's observation, and a good one: the relay must be an **acceptor** toward one peer and an **initiator** toward the other, so **building it necessarily produces a third implementation carrying both roles** — exactly the slack the paragraph above says does not exist. That is an argument for building the relay **early rather than last**, and for it living where both teams can point at it, which is the decision already taken.
...
⛔ **And a sixth:** the transcript bound into `sid` or `PRK` as well, against [11.6c1](#116-derivation).

⛔ **Two of the six produce a *successful-looking* pairing — the first and the sixth — and they are indistinguishable until the `PRK` is compared.** `sid` feeds only `PRK`; `sas_raw` and `K_c` descend from `BK` and the transcript. So both of those causes agree on all six digits **and** on both MACs, the operator affirms a comparison that genuinely succeeded, and the failure surfaces one connection later as a TLS error. Every other cause in the list breaks the digits or the MACs and is caught before anyone affirms anything.

*Erratum E44, 24 August 2026 — finding R-12, PinPointStudio.* This paragraph said the sixth was the **only** cause with that signature, which is wrong and wrong in the costly direction: the paragraph exists to triage a `PSK_IDENTITY_NOT_FOUND`, its whole job is to be read by someone whose pairing succeeded and then failed, and it routed them to the newest cause and away from the older, more familiar one. **Cause 1 predates E34** and is the mistake the two-row `expand`/`sid` presentation above was built to catch in the first place.

### Counter-vectors

*Added by erratum E44. The values a **wrong** implementation produces, so a reproduction is self-checking rather than merely matching.* [§10.3](#103-pairing-code) publishes a worked example; these publish the three near-misses, on the same argument that put the little-endian misread in the list above.

```
cause 1 — sid salted BEFORE its version and variant bits are set (11.6d)
  PRK    9b77924572627d0e6d1c51fc679a3596ccd1c4a7dff7943da2ef856ef64dc1ba
  digits 435948 and both MACs are CORRECT — this is the silent one

cause 6 — transcript bound into sid as well, against 11.6c1
  sid    18dd04b1da8342a6b4248fb1bd2d0626
  digits 435948 and both MACs are CORRECT — this is the other silent one
  differs from the true sid in its FIRST octet, so one printed line settles it

R-11 witness — X25519 is not contributory (11.6c2)
  pk_a'  87abc1e84c4c5572d2b1e63c69f5617a215518cf6261eb5a0e7db49ddad34208
  X25519(sk_i, pk_a') = 7c79d7b5f31b9aac367477f5f7c7a68b5c44cac28ed5c902a59ec48c02956a6a
                      = Z exactly, from a DIFFERENT public key, and non-zero
  BK, sid and PRK are therefore identical under the substitution;
  only sas_raw separates the two peers:  435948  (pk_a)   vs   485158  (pk_a')
```

### The interposer quadruple

*Added by erratum E54, finding R-22. What [RT-20a](#9-conformance) runs on, and it is published **with both shared secrets** so that the row needs no key agreement — [11.11c](#1111-where-x25519-comes-from)'s promise would otherwise not reach the one test that touches the security property.*

An interposer holds two keypairs, one per leg. `sk_i` and `sk_a` are [§10.4](#104-guided-pairing)'s own; `v` is `1`.

```
sk_M1  a0a1a2a3a4a5a6a7a8a9aaabacadaeafb0b1b2b3b4b5b6b7b8b9babbbcbdbebf
sk_M2  c0c1c2c3c4c5c6c7c8c9cacbcccdcecfd0d1d2d3d4d5d6d7d8d9dadbdcdddedf
pk_M1  605a725d2a4adfeeb1a29e17edd621c1b7593ee8cdbc44ac6c4ab6e2f805d23c
pk_M2  dc2cca31e8e43bbd91dff7e475cca3347eb478107d5bd765aba4ae4a30c35d44

leg 1 — the initiator believes it is talking to the interposer
  Z1 = X25519(sk_i, pk_M1) = X25519(sk_M1, pk_i)
       0be5bbbe4a543727f7868cfa105122ca94bc39c1521a3b48dd149d9b726bc312
  sas_raw = HKDF-Expand(HKDF-Extract("ppcp1 bootstrap", Z1),
                        "ppcp1 sas" || 01 || pk_i || pk_M1, 4)
          = aa480ea7   ->   849063

leg 2 — the acceptor believes the interposer is the initiator
  Z2 = X25519(sk_M2, pk_a) = X25519(sk_a, pk_M2)
       caec775fe4759a50d53cfaaac27866b18d8acce940c34a8c844ab9b062aba02e
  sas_raw = HKDF-Expand(HKDF-Extract("ppcp1 bootstrap", Z2),
                        "ppcp1 sas" || 01 || pk_M2 || pk_a, 4)
          = e668025b   ->   576027

849063  ≠  576027        the operator sees two different numbers
```

⚠ **The quadruple does not demonstrate the security property on its own, and RT-20a says so.** It shows the *derivation* separates two legs for **these** keys. What makes that meaningful is that the attacker had to choose both of its keys **blind** — and that is [11.5c](#115-the-exchange)'s ordering, which no amount of arithmetic can observe. It is [RT-20b](#9-conformance)'s, and RT-20b needs the relay.

⛔ **The third block is the one to run before deciding [11.6c2](#116-derivation) is over-cautious.** It takes one call to any X25519 implementation, and it is the difference between believing the explicit key binding is redundant and knowing it is not.
...
⚠ **The premise CR-01 was raised on does not hold, and that is recorded here rather than left to be inherited.** *Erratum E53, 24 August 2026.* The request asked for *"a venue where a range operator sets up several bays"*, and three review passes then built on it: [3.6a](#36-multicast-is-not-to-be-relied-on) says multicast **will not work at a range**, so RV-6 reached over mDNS looked like a feature that worked in an office and not where it had been asked for, and [3.7h](#37-the-bootstrap-window) looked like the clause that decided whether it was worth building.

**None of that arises.** The protocol owner has confirmed the deployment: **the host is never at the range.** A capture device at a range works standalone and its session travels home as a bundle ([`PPCP-CORE` §9](ppcp-core.md#9-offline-sessions-and-bundles), [`PPCP-ENC` §7](ppcp-encoding.md#7-bundle-container)); the two peers meet on a **home or coaching-studio network**, which is one the user controls and where multicast behaves. **The range is not a two-peer environment, so there is no venue at which RV-6 needs to work and does not.**

[3.6a](#36-multicast-is-not-to-be-relied-on) stands as written — it is a general caution about multicast on any network and it earns its place. [3.7h](#37-the-bootstrap-window) stands as the MAY it always was. What is withdrawn is the claim that either of them bounded this feature's usefulness.

⛔ **The lesson is worth more than the correction.** RV-6 was granted, then reviewed four times by two teams, producing twenty findings — **and not one of them asked whether the deployment the feature was for actually existed.** Both teams inherited the premise from the change request, and so did I. The reviews checked the answer with great care and never checked the question. See [the change-request process notes](../changerequests/README.md).

**RV-6 is kept**, on a narrower and more honest basis than the one it was granted on: comparing six digits needs no camera, no focus and no line of sight between two screens, and it works when one of the two is a desktop monitor that cannot be pointed at anything. That is a real improvement on scanning a code for one phone and one PC in one room, which is the deployment that exists.

⚠ **Five review passes, and the last two found nothing in the clauses.** [§4](#4-rv-2--the-pairing-code) survived three passes and three independent recomputations of its vectors; this has had five of each. Passes 2 and 3 each found a defect the previous pass's own fix had introduced ([E40](#errata-after-revision-9--change-request-cr-01-and-its-review), then [E43](#errata-after-revision-9--change-request-cr-01-and-its-review)); pass 4 found none; **pass 5 ran [RT-20a](#9-conformance) for the first time and found the property sound and the number describing it 4.86% wrong** ([E54](#errata-after-revision-9--change-request-cr-01-and-its-review)). Its vectors now reproduce byte for byte on two implementations sharing no code, on both sides of the E34 boundary. **[RT-20](#9-conformance) — the test the path exists to pass — still cannot run**, because it needs two real implementations either side of a deliberate relay and neither has written this section yet. Nothing in two review passes has changed that, and it should not be allowed to feel as though it has.
...
- **(11.4a) MUST** A bootstrap frame is framed exactly as [`PPCP-ENC` §3](ppcp-encoding.md#3-framing) — the 8-byte header, then a deterministically encoded CBOR map ([4.3a](#43-payload)) — with the header's `channel` byte set to **`255`**, which [`PPCP-ENC` 2a](ppcp-encoding.md#2-channels) reserves and no PPCP channel may use.
  ...
  ⛔ **An implementation trap, stated because it is a plausible first act and it is exactly backwards.** A PPCP channel validator **rejects 255 and must go on rejecting it** — that rejection *is* the fail-closed property this clause depends on. An implementer meeting it while adding [§11](#11-rv-6--guided-pairing), and concluding the validator needs relaxing to let a bootstrap frame out, would delete the safety argument in the course of implementing the clause that relies on it. **A bootstrap frame is written by a separate path that does not consult the PPCP channel rule**, because a bootstrap connection is not a PPCP link ([1.3c1](#13-where-it-stops)).
...
- **(11.4c1) MUST** *Erratum E46, 24 August 2026 — finding R-15, PinPointStudio.* A peer **aborts with `malformed` on a map key it does not recognise** in any bootstrap frame. Bootstrap frames are a **closed** vocabulary, and this is the one place in the protocol set where that is so: [3.3a](#33-txt-record) has a TXT receiver ignore unrecognised keys, [4.2c](#42-version-handling) has a pairing-code decoder do the same at every nesting level, and [A4](#annex-a--decisions-and-alternatives) chose CBOR partly for that tolerance. **[§11](#11-rv-6--guided-pairing) is the exception**, matching [11.10a](#1110-what-must-not-cross-a-bootstrap-connection)'s *"the five frames of [§11.4](#114-frames) are its entire vocabulary"* and the fail-closed posture of the reserved channel byte.

  Two implementations could have differed here without either being wrong, and [§10.4](#104-guided-pairing) could never have shown it — the [RT-18](#9-conformance) category again, and the third instance of it in three passes. ⛔ **The choice is not cosmetic: it decides whether a future `v2` can extend a v1 frame.** Rejecting means it cannot, so [B18](#annex-b--open-issues)'s negotiation question — if it is ever answered *yes* — is a v2 designed with it from the start rather than a field added to what exists. That consequence is accepted deliberately, and it is stated here rather than discovered when someone tries.
...
- **(11.6c2) MUST NOT** *Erratum E43, 24 August 2026 — finding R-11, PinPointStudio.* An implementation **omit `pk_i` and `pk_a` from [11.6c](#116-derivation)'s transcript on the grounds that `Z` already depends on them.** It does not follow, the saving looks like a simplification, and **removing them is undetectable from outside** — every value in the exchange stays well-formed and the property [§11.8](#118-what-the-comparison-proves) rests on is gone.

  **X25519 is not contributory.** Clamping forces every scalar to a multiple of 8, so for any point `T` whose order divides 8, `k·(P + T) = k·P`: a counterpart can present a **different** 32-byte public key that yields a **bit-identical, non-zero** shared secret. [11.6b](#116-derivation) does not fire — the agreement succeeds and `Z` is not zero — because it rejects a key that is *wholly* small-order, and a legitimate key plus a small-order component is not that. [§10.4](#104-guided-pairing) carries the witness, computed from its own keys and reproduced on two implementations including OpenSSL 3.6.3, which accepted the substituted key without complaint.

  Follow the substitution through and every value [11.6c1](#116-derivation) says needs no transcript — `BK`, `sid`, `PRK` — is **identical** across it. **The only thing that separates the two peers is `sas_raw`'s explicit `pk_i || pk_a`.** That binding is load-bearing, not belt-and-braces, and this clause exists because the sentence deleted by [E43](#errata-after-revision-9--change-request-cr-01-and-its-review) told a reader the opposite. It is [11.5c](#115-the-exchange) and [A15](#annex-a--decisions-and-alternatives)'s shape — a clause whose removal reads as a tidy-up and is fatal — arriving through a *rationale* rather than through a clause, which is the failure [E37](#errata-after-revision-9--change-request-cr-01-and-its-review) was raised for one pass earlier and [E40](#errata-after-revision-9--change-request-cr-01-and-its-review) one pass after that.

  ⚠ **This is not an attack and needs no new check.** The substitution gives an interposer nothing — it cannot compute `Z` without a private key, and the digits diverge, so the operator catches it. The cofactor behaviour is inherent to X25519. **The defect was the sentence.**
...
- **(11.6c1) MUST NOT** *Erratum E40, 24 August 2026 — finding R-09, PinPointStudio.* The transcript be bound into [11.6d](#116-derivation)'s `sid` or [11.6e](#116-derivation)'s `PRK`. **Those two are functions of `Z` alone, deliberately**, and an implementation that binds the transcript into either does not interoperate with one that does not.
  ...
  **Why the boundary falls where it does.** By the time `PRK` is derived the exchange has already been authenticated — by the comparison, and by the MACs — so binding the transcript again adds nothing: **the transcript is bound where it is *checked*, and `sid` and `PRK` are downstream of both checks.** Two peers that reach `PRK` have already proved they agree on `v`, `pk_i` and `pk_a`. And [§5.1](#51-key-derivation) is taken **verbatim** by [11.6e](#116-derivation): changing its inputs from this section would give §5.1 a second shape, which is [A17](#annex-a--decisions-and-alternatives)'s argument for tearing the bootstrap connection down rather than upgrading it, arriving one layer lower.

  *Erratum E43, 24 August 2026 — finding R-11, PinPointStudio, and independently the precision point in PinPointCapture's third pass.* This paragraph rested on a third leg until now — that **`Z` already commits to both public keys by construction** — and that claim is **false**. It is deleted rather than repaired, because the argument above carries on its own and because the claim pointed at the one thing actually holding the property up. See [11.6c2](#116-derivation).
...
- **(11.6f) MUST** A peer erases its ephemeral private key, `Z`, `BK` and `K_c` when the handshake ends, **whether it succeeded or failed**, and they appear in no log or export ([7.2e](#72-handling-the-pairing-secret)). What survives a successful handshake is `PRK` and what [§5.1](#51-key-derivation) derives from it; what survives a failed one is nothing. **On a failed or aborted handshake that erasure includes `PRK`, `K_tls`, `K_id` and `sid` where they were computed**, and the digits with them ([11.7f](#117-the-short-authentication-string)).

  *Erratum E51, 24 August 2026 — finding F-R9-5, PinPointCapture.* The closing sentence always carried the failure case; the **explicit list did not**, and the list is what gets implemented from. A peer computes the whole chain the moment it holds `Z` — before either user has affirmed anything, and [11.3e](#113-roles-and-the-connection) allows sixty seconds for that — so on every abort path there is a `PRK` in memory for a pairing that does not exist and never will. [11.5g](#115-the-exchange) says the pairing does not exist until both sides affirm and both MACs verify; **computing is not holding, and this clause is what closes the gap between them.**
...
**The modulo bias is real, immaterial, and recorded here so it is not raised twice more.** 2³² is not a multiple of 10⁶, so **967 296** of the residues have 4 295 preimages and the remaining **32 704** have 4 294 — an excess over uniform of **7.6 × 10⁻⁶** for the most probable string, and a factor of **1.000 23** between the most and the least probable. It cannot be steered by an attacker who commits blind ([§11.8](#118-what-the-comparison-proves)) and it does not move the security bound by anything worth writing down — [E54](#errata-after-revision-9--change-request-cr-01-and-its-review) computes the exact figure and shows the bias cancelling to seven significant figures.

*Erratum E42, 24 August 2026 — finding R-07, PinPointStudio.* This paragraph carried three wrong numbers until now, and its stated purpose is that *the third reader does not spend the same afternoon*: `295 967 296` is not a possible count of residues when there are only 10⁶ of them, the favoured set is the **large** one rather than *"the rest"*, and the excess was given as 2.3 × 10⁻⁷ — **thirty-three times smaller than it is**. The 1.000 23 figure was right and was attached to the wrong quantity: it is the most-to-least ratio, not the excess over uniform. The conclusion is unchanged in every respect, which is why it took a second pass to notice.

⚠ **The attribution in this note was itself wrong until a third pass, and is corrected here.** E42 recorded that two of the three figures came from PinPointStudio's pass-1 review. **They came from PinPointCapture's.** `295 967 296` and the inverted *"and the rest"* are verbatim from PinPointCapture's N1; only `2.3 × 10⁻⁷` was PinPointStudio's. PinPointStudio's own pass-1 figure was **`967 296` — the correct count, in the correct direction** — and this paragraph as published took the wrong count over their right one. PinPointStudio then accepted the blame in pass 2 without checking, and PinPointCapture corrected the record against itself in pass 3. **A misattributed error is one the wrong team looks for next time**, which is the same reason the little-endian figure was worth fixing rather than leaving.
...
[11.5b](#115-the-exchange) and [11.5c](#115-the-exchange) are what deny it that. The initiator commits to `pk_i` before seeing anything; the acceptor reveals `pk_a` having seen only a hash. So on the acceptor's leg the attacker must choose its key before learning `pk_a`, and on the initiator's leg it is bound by a commitment it made before learning `pk_i`. Neither leg's digits can be steered. The attacker's best strategy is to pick both keys blind and hope the two legs collide — **one chance in 1 000 000** — and a miss is a mismatch on two screens with an operator looking at both.

*Erratum E54, 24 August 2026 — findings R-21 (PinPointStudio) and F-R9-6 (PinPointCapture), raised independently and computed identically.* This read **1 048 576** until now, which is 2⁻²⁰ — **the width of `sas_raw`, not of what is displayed.** [11.7a](#117-the-short-authentication-string) reduces it mod 10⁶ before anyone sees it, and the attacker must match the **digits**, so the space is 10⁶. Exactly, including the modulo bias:

```
p(collision) = 144115188323 / 144115188075855872  =  1.0000000017 × 10⁻⁶   =  1 in 999 999.998
2⁻²⁰                                              =  9.536 743   × 10⁻⁷   =  1 in 1 048 576
```

**The real bound is 4.86% weaker than the one this document stated**, which is immaterial to the argument — one in a million and one in 1.05 million are the same claim in every way that matters, and [§11.8](#118-what-the-comparison-proves)'s force is the *single attempt*, not the width. It was material to [RT-20a](#9-conformance), which told an implementer to check a measured rate against a number 4.86% away from the truth.

✅ **And it settles something [E42](#errata-after-revision-9--change-request-cr-01-and-its-review) could not.** The modulo bias contributes `1.7 × 10⁻⁹` relative — it cancels to seven significant figures against uniform-over-10⁶, so **the bias does not reach the security bound at all.** That is what [§11.7](#117-the-short-authentication-string)'s paragraph claimed and could not previously show.
...
- **(11.9d1) SHOULD** *Erratum E45, 24 August 2026 — finding R-14, PinPointStudio; endorsed by PinPointCapture.* A peer aborting with **`unsupported_version`** offers the pairing code on the **first** abort rather than the second. A second attempt is guaranteed to fail identically — [11.4h](#114-frames) has the initiator offer the highest `v` it implements and forbids proposing lower — so [11.9d](#119-aborting-and-the-one-attempt-rule)'s two-abort threshold spends an operator's attempt on a certainty.

  **This converts the one real gap in [§11](#11-rv-6--guided-pairing)'s version handling from *permanently unpairable* into *pairs by code, this once*.** RV-6 offers `v` as a single value, so a **newer initiator meeting an older acceptor cannot complete a guided pairing at all**, though both implement the older version. The asymmetry runs one way only: [11.4h1](#114-frames) has the acceptor echo, so a newer *acceptor* pairs happily with an older initiator. ⚠ **On the deployment CR-01 was raised for, the stuck direction is the normal one** — the host is the initiator and is updated whenever the user likes; the capture peer is the acceptor and its rollout lags. *"Host updates first, phones catch up over the following weeks"* is exactly the case that breaks, and it is the default way these two ship.

  Two things follow and neither is a clause. **Where two implementations bump `v`, the acceptor side ships first** — nobody will deduce that from [11.4h](#114-frames) and [11.4h1](#114-frames) sitting next to each other, and it prevents the whole failure. And whether RV-6 should have version *negotiation* rather than *assertion* is recorded at [B18](#annex-b--open-issues) rather than answered here: both teams argued against adding it, on the grounds that the fallback is the REQUIRED path ([2a](#2-rendezvous-paths)) which both measured at 30/30, and that [11.6g](#116-derivation) and [A16](#annex-a--decisions-and-alternatives) both hold agility on a first-contact handshake to be a downgrade surface. Trading a permanent attack surface for a temporary inconvenience is the wrong way round.
...
### 11.11 Where X25519 comes from

*Erratum E48, 24 August 2026 — closing [B17](#annex-b--open-issues). Stated in octets, not as an interface: the shape of the interface is each implementation's own, and this document does not have one. The `libppcp` surface the two applications compile against is recorded separately, in [CR-01's X25519 seam note](../changerequests/CR-01-x25519-seam.md), which is an API decision and not a protocol one.*

[§11](#11-rv-6--guided-pairing) needs exactly one primitive the rest of this protocol set does not. SHA-256, HMAC-SHA256 and HKDF-SHA256 are already required by [§5.1](#51-key-derivation) and are short enough that an implementation reads them and a reviewer checks them. **X25519 is constant-time field arithmetic over 2²⁵⁵−19 and is not.** An implementation whose licence or dependency constraints forbid a cryptographic library can neither vendor it nor safely write it — and one implementation of this document is in exactly that position, which is why the boundary is specified rather than left to be invented three times.

- **(11.11a)** Exactly **two values** in this section require key agreement: a peer's own **public key** `pk`, and the **shared secret** `Z`. Each is 32 octets.
- **(11.11b) MUST** Everything else is SHA-256, HMAC, HKDF and CBOR — the commitment ([11.5b](#115-the-exchange)), `BK`, `sas_raw`, `K_c`, both MACs, `sid` and `PRK` ([§11.6](#116-derivation)), and the framing ([§11.4](#114-frames)). **No part of it requires X25519.**
- **(11.11c) MUST** The derivation of [11.6c](#116-derivation) through [11.6e](#116-derivation) is a **pure function of `Z`, `v`, `pk_i` and `pk_a`**, and is implementable by a component with no key-agreement capability whatever. That is what lets [§10.4](#104-guided-pairing) and [RT-18](#9-conformance) be reproduced by such a component, given the published `Z` — and it is the same reason every random value in this document is a *parameter* of the computation rather than something the computing component produces.
- **(11.11d) MUST** Where the two values are supplied across a boundary, **only those two cross it.** `BK`, `sas_raw`, the digits, `K_c`, either MAC, `sid` and `PRK` do not. **The boundary is below the derivation, not inside it.**
- **(11.11e) MUST** The private scalar is 32 octets from the CSPRNG [7.2a](#72-handling-the-pairing-secret) already makes the embedding's obligation, and is **not clamped** before it crosses. [RFC 7748](https://www.rfc-editor.org/rfc/rfc7748) clamps internally and clamping is idempotent, so a supplier that clamps as well is conformant and one that relies on X25519 to clamp is conformant. Stated because it is otherwise an interoperability question with no observable answer until two implementations disagree — the class [E39](#errata-after-revision-9--change-request-cr-01-and-its-review), [E41](#errata-after-revision-9--change-request-cr-01-and-its-review) and [E46](#errata-after-revision-9--change-request-cr-01-and-its-review) each landed in.
- **(11.11f) MUST** Whoever performs the agreement **reports failure distinguishably from success**, and the caller treats a reported failure and an all-zero `Z` **identically**, aborting with `invalid_key` ([11.6b](#116-derivation)). This is where [E36](#errata-after-revision-9--change-request-cr-01-and-its-review)'s two measured behaviours converge — OpenSSL fails the call, CryptoKit throws, and something else may yet return zeros — and a caller cannot know which it has. ⛔ **A boundary that dropped the failure signal, or reported it as a transport error, would make [11.6b](#116-derivation) unimplementable on the far side of it.**
- **(11.11g) MUST** The agreement is **constant-time with respect to the private scalar**. This is the reason the boundary exists at all, and it is an obligation on the supplier that nothing downstream can check.
- **(11.11h) MUST** The private scalar and `Z` are erased by whichever component holds them, when the handshake ends, whether it succeeded or failed ([11.6f](#116-derivation), [7.2e](#72-handling-the-pairing-secret)). A component that never held them has nothing to erase, which is half the point of putting the boundary here.
- **(11.11h1)** *Erratum E51, 24 August 2026 — PinPointCapture, fourth pass.* **On at least one platform [11.11h](#1111-where-x25519-comes-from) is partly an obligation on a closed-source framework, and that is recorded rather than assumed away.** Where the key agreement is supplied by a system framework that both generates and retains the private scalar and returns the shared secret as its own type, the implementation can guarantee it holds no copy of either; it cannot guarantee the framework holds none, and it cannot verify that from outside. `CryptoKit`'s `Curve25519.KeyAgreement.PrivateKey` and `SharedSecret` are the measured case — neither exposes a zeroise, and Apple documents zero-on-release for `SymmetricKey` but not for those two. The obligation stands as written because the alternative is worse and because the mitigation is real: derive into a type that *is* documented to zero, and let the framework's own hold go out of scope immediately. **This is the same class as [§5.4](#54-resolved-the-mechanism)'s platform limits — not a defect, a bounded truth about what a MUST can mean here** — and [RT-23](#9-conformance) records the bound rather than reporting a pass it cannot justify.
- **(11.11i)** An implementation with native X25519 needs **no** boundary, and this section constrains nothing about it. It is written for the case where one component derives and another computes.

**Why the boundary falls between `Z` and everything downstream of it, rather than at a key-agreement callback.** A callback is the right shape when the computing component needs something it cannot produce *during* a loop it owns — which is why [5.3a1](#53-psk-identity)'s rejection sampling takes one. Key agreement has no loop: the component holding the private scalar already has it, and already has the counterpart's public key off the wire, so it computes `Z` once and hands over 32 octets **exactly as it hands over `psk`, `sid` and `rn`**. Nothing calls back, no lifetime is shared, and the deriving component's dependency list does not change.

The gain is not only tidiness. [11.11c](#1111-where-x25519-comes-from) means **the entire derivation chain, and every row of [§10.4](#104-guided-pairing) below `Z`, can be tested by something that cannot do elliptic-curve arithmetic at all** — which is what makes [RT-18](#9-conformance) runnable in the one component both applications share, rather than twice over in two applications that would then be agreeing with themselves ([B7](#annex-b--open-issues)).
...
| **A18** | **Host advertises, capture device browses and dials — as the deployment's committed shape, not merely its permitted one** ([3.5e](#35-who-advertises-and-who-browses)) | Have the host browse and dial the capture device, which is what *"the host finds the phone"* sounds like it should mean | [3.5d](#35-who-advertises-and-who-browses) forbids the alternative outright — a capture peer that cannot resolve a PSK identity server-side can never be the TLS server, on **any** path in this document, so it can never be the listener and therefore never the advertiser. But the decision does not rest on that. A **foregrounded** application dials reliably; a **backgrounded** one cannot reliably listen, and capture needs the foreground anyway. And it matches the workflow: the host is set up first and already advertising, and the device joins when it comes online. **The platform constraint and the ergonomics agree**, which is why this is recorded as a decision rather than as a concession. First contact under [§11](#11-rv-6--guided-pairing) runs the other way — no PSK, so no constraint — and that is where *"the host finds the device"* is genuinely available. |
...
| ~~**B18**~~ | **Closed as decided, 24 August 2026 — negotiation is deliberately not added**, and [11.9d1](#119-aborting-and-the-one-attempt-rule) bounds the consequence to *pairs by code, this once*. Both teams argued against it and the reasoning is below; [11.4c1](#114-frames) makes any future answer a v2 designed with it from the start rather than a field added to what exists. ~~**RV-6 has version *assertion*, not version negotiation.**~~ `bs_offer.v` is a single value, so a **newer initiator cannot pair with an older acceptor** even where both implement the older version — and on this deployment that is the normal ship order, the host being the initiator and the faster-moving side. [11.9d1](#119-aborting-and-the-one-attempt-rule) bounds the consequence to *pairs by code, this once*. | **Open, and deliberately not answered.** Both teams argued against adding negotiation and the reasoning is recorded: the fallback is the REQUIRED path that both measured at 30/30, and [11.6g](#116-derivation) and [A16](#annex-a--decisions-and-alternatives) both hold agility on a first-contact handshake to be a downgrade surface — trading a permanent attack surface for a temporary inconvenience is the wrong way round. ⛔ **The naive fix is unsafe and this is the record of why**: making `v` a list lets an attacker strip the high entries, the acceptor picks honestly, both peers bind the selection, and the digits match. The only safe shape binds **the entire offered set together with the selection** into [11.6c](#116-derivation)'s transcript — TLS 1.3's transcript logic, which E34's machinery would support — and it needs a canonical encoding of the offered set, which is the part most likely to diverge and would want its own [§10.4](#104-guided-pairing) row. [11.4c1](#114-frames) makes this a **v2-designed-from-scratch** matter rather than a field added later. |
| ~~**B17**~~ | ~~[§11](#11-rv-6--guided-pairing) needs a primitive the protocol set has never needed before, and an implementation may have nowhere to put it.~~ | **Closed 24 August 2026 by [§11.11](#1111-where-x25519-comes-from) ([E48](#errata-after-revision-9--change-request-cr-01-and-its-review)).** The boundary is stated in **octets** — two 32-octet values, `pk` and `Z`, and nothing else — rather than as an interface, which keeps it language-neutral and leaves each implementation its own shape. It is a **parameter**, not a callback: key agreement has no loop, so the component holding the private scalar computes `Z` once and hands it over exactly as it already hands over `psk`, `sid` and `rn`. The consequence worth having is [11.11c](#1111-where-x25519-comes-from) — the whole derivation is a pure function of `Z` and the transcript, so [§10.4](#104-guided-pairing) is reproducible by a component that cannot do curve arithmetic at all. The concrete API is [recorded separately](../changerequests/CR-01-x25519-seam.md); **nothing is implemented, and no session is open.** |
| **B14** | **X25519 reachability through both platforms' public interfaces is assumed, not measured.** [11.6a](#116-derivation) mandates it and [A16](#annex-a--decisions-and-alternatives) argues for it on the belief that CryptoKit's `Curve25519.KeyAgreement` and the host's crypto library both expose raw X25519 with no TLS involved. That belief is the same *shape* as the one [§5.4](#54-resolved-the-mechanism) held about TLS 1.3 external PSK for four drafts before anyone ran it. | ~~**B14**~~ | ~~X25519 reachability is assumed, not measured.~~ | **Closed 24 August 2026, on hardware at both ends.** PinPointStudio measured it against OpenSSL 3.6.3; PinPointCapture against CryptoKit on an **iPhone 16 running iOS 26.6** — RFC 7748 §6.1 reproduces, [§10.4](#104-guided-pairing)'s `Z` matches, and all three small-order public keys are rejected by **throwing** rather than by returning zeros, which is [11.11f](#1111-where-x25519-comes-from)'s *throw half* confirmed on a device rather than on a proxy. **[5.4b](#54-resolved-the-mechanism)'s rule is satisfied rather than waived** — this document had already paid once for accepting a desktop stand-in for a device measurement, and did not do so again. PinPointCapture's correction is recorded with it: the analogy to the TLS 1.3 PSK gap does **not** hold, that one being structural where this is a first-class primitive with raw bytes in and out. |
| ~~**B15**~~ | **Closed as decided, 24 August 2026 — not served, and deliberately.** Its prerequisite is [B2](#annex-b--open-issues)'s per-peer re-keying inside the authenticated channel, which remains open; specifying a venue or group credential before that exists would ship [7.4f](#74-persistent-pairings)'s exposure permanently rather than for one session. What follows is the record of what the case actually is, since [E53](#errata-after-revision-9--change-request-cr-01-and-its-review) changed it. ~~**The fleet case is not served, and its real motivation is not the one CR-01 gave.**~~ *Restated by erratum E53:* CR-01's *"a range operator sets up several bays"* **does not describe this deployment** — the host is not at the range. What survives, and is genuine, is PinPointCapture's case: **multi-device stereo puts two or three phones on one host**, each with a screen — so [11.7b](#117-the-short-authentication-string) is satisfied and the fleet problem arrives anyway, in a **studio** rather than at a venue, and at a scale of two or three rather than a bay per operator. [§11](#11-rv-6--guided-pairing) still costs one confirmation per device. What would collapse that to one per device is a venue-scoped enrolment credential — pair once with the venue, connect to any bay in it — and that is a **group credential**, which is exactly what [7.4f](#74-persistent-pairings) forbids persisting and what [B2](#annex-b--open-issues) says has no revocation story. | **Open, and deliberately not attempted here.** Its prerequisite is [B2](#annex-b--open-issues)'s per-peer re-keying inside the authenticated channel, not a fourth rendezvous path. Specifying a venue credential before B2 exists would ship the multi-device exposure [7.4f](#74-persistent-pairings) was written to bound, permanently rather than for one session. |
| ~~**B16**~~ | ~~[§11](#11-rv-6--guided-pairing) has had no implementation review pass.~~ | **Closed 24 August 2026.** Both teams reviewed it and both accept the ruling. Six findings, two of them blocking and both structural — an unbound version field ([E34](#errata-after-revision-9--change-request-cr-01-and-its-review)) and an unserialised initiator ([E35](#errata-after-revision-9--change-request-cr-01-and-its-review)) — and the vectors reproduced byte for byte on both sides by implementations sharing no code, which is what the ask was for. **The pass did exactly what [§4.3b](#43-payload)'s did**: the defects it found were invisible in the worked example and would have been permanent after either team shipped. **A second pass over the amendments then found [E40](#errata-after-revision-9--change-request-cr-01-and-its-review)**, a trap created by the first pass's own fix, **and a third found [E43](#errata-after-revision-9--change-request-cr-01-and-its-review)**, a false claim in the rationale of that fix. Three passes, and each found a defect the previous one had introduced — which is the argument for reviewing errata and not only the text they amend, and for reviewing the *explanations* as carefully as the clauses. |
...
## E57 — the second one, and the friction it was buying

**7.4b is a SHOULD.** It required a persisted pairing to be opt-in, visible to the user, and individually revocable.

⛔ **The objection is to *opt-in*, and it is a product objection rather than a security one.** A user who is deliberately pairing a device **has already said what they want.** Asking them a second time, before the pairing may be remembered, puts a consent step inside a flow whose entire purpose was consent — they scanned a code, or compared six digits and affirmed them, and are then asked whether they meant it. It informs no decision they have not made, and both implementations landed in the same place because of it: a default-off toggle present on one screen out of two, and a paired device quietly forgotten when the session ended. The clause bought friction and no safety.

**The general point is the one that decides it, and it is [E56](#errata-after-revision-9--change-request-cr-01-and-its-review)'s point one clause along.** It is overstepping for a protocol to tell an application how to behave. Opt-in, visible and individually revocable is a statement about **screens**; [§1.3](#13-where-it-stops) already lists user interface among the things this document does not decide; nothing in the clause is observable between peers, so two implementations — one with a revocation list and one with none — produce byte-identical traffic and interoperate perfectly; and the reference library cannot implement a line of it. **A clause no counterpart can detect and no library can carry was never this document's to require.**

⚠ **What it costs, and it must not be read as a small thing.** [E56](#errata-after-revision-9--change-request-cr-01-and-its-review) relaxed [7.2c](#72-handling-the-pairing-secret) **partly on the strength of 7.4b remaining a MUST** — that paragraph called it *the control that survives*. It did not survive the day. **After E57 there is no normative requirement protecting a persisted `PRK`**: not where it is kept, and not that a user can see or end it.

⛔ **And one seam worth naming.** [7.4d](#74-persistent-pairings) is untouched and still a MUST: a revocation, once it happens, is honoured immediately and fails the counterpart's handshake. Nothing now requires that a way to revoke **exists** — and [7.4a](#74-persistent-pairings) gives a persisted pairing no expiry, so a conformant peer may hold one its user can neither see nor end, for as long as it runs. What still binds is narrow: [7.4c](#74-persistent-pairings) scopes a pairing to its counterpart and [7.4f](#74-persistent-pairings) forbids persisting one from a multi-use code. **The clause is kept as advice rather than deleted, because it is still the better product** — what changed is who is entitled to insist on it.

## After CR-01 — E56, and a MUST that was never ours to make

**[7.2c](#72-handling-the-pairing-secret) is a SHOULD.** It required secrets at rest to sit in the platform's protected storage, and [§1.3](#13-where-it-stops) says in as many words that *platform permission handling* and *how a peer stores its own secrets at rest* are out of scope. The clause reached into its own stated exclusion and made the decision anyway — and on most platforms that decision costs a **permission prompt the application has to justify to its user**, which is not a cost this document is in a position to spend.

⛔ **Both implementations were broken by it, in opposite ways, and neither reported it as a defect for a month.** One offered persistence on a single platform of three, because *"where one exists"* is honest and on the other two nothing does — so the pairing was forgotten on every launch and [§7.5](#75-reconnecting-within-a-session) had nothing to reconnect to. The other stored its secrets in a class that cannot be read while the phone is locked, so the sweep found nothing and said *no pairings held*, which is a different statement and an untrue one. **Neither implementation was wrong.** Both did what the clause said.

**This is [E3](#errata-after-revision-9--change-request-cr-01-and-its-review) again and it should be uncomfortable that it is.** E3 was a clause counting the wrong thing, killing [§7.5](#75-reconnecting-within-a-session) in the default case, found by an implementation. This is a clause mandating the wrong thing, killing [§7.4](#74-persistent-pairings) on most platforms, found by two. **The recurring failure is not in the clauses — it is that a requirement whose cost lands entirely outside this document gets written as though it were free.** 7.2a costs nothing to satisfy and is a correct MUST. 7.2c cost a permission, a platform port, or a locked-device failure, and was written in the same voice.

⚠ **The honest reading of what this gives up.** A `PRK` in an ordinary settings file is readable by anything running as that user. [7.4a](#74-persistent-pairings) was always a MAY and is now a weaker one, and [7.4b](#74-persistent-pairings)'s opt-in, visible, individually revocable persistence is the control that survives — the weaker-looking half of the pair, briefly the load-bearing one. **No compensating MUST was added**, deliberately: every candidate for one was another storage decision, which is the thing this erratum exists to stop making. ⛔ **And [E57](#errata-after-revision-9--change-request-cr-01-and-its-review), the same day, made 7.4b a SHOULD too** — so the *control that survives* did not survive long. Read the two as one movement: the specification stopped making product decisions on the application's behalf, and stopped protecting the persisted key while doing it.

## The final pass — errata E54–E55, and CR-01 closes

**Both teams: no objection to implementation.** Four paper items, no new design, and one of them is the first execution of the only test that touches the security property.

⛔ **[RT-20a](#9-conformance) was run, and it found the number describing the property wrong.** PinPointCapture executed it the same afternoon [E50](#errata-after-revision-9--change-request-cr-01-and-its-review) split the row — after four passes reporting RT-20 as unmoved — over 200 000 random interposer quadruples: **zero collisions, digits uniform at χ² 933.6 over 999 dof.** The property held. **The bound did not**: this document said *one chance in 1 048 576*, which is 2⁻²⁰, **the width of `sas_raw` rather than of what is displayed.** [11.7a](#117-the-short-authentication-string) reduces mod 10⁶ first, so the space is 10⁶ and the true figure is `144115188323/144115188075855872` — **4.86% weaker than published**. Both teams found it independently and computed the same exact fraction.

It changes no argument: one in a million and one in 1.05 million are the same claim, and [§11.8](#118-what-the-comparison-proves)'s force was always the *single attempt* rather than the width. It changed a conformance row, which had been told to check a measurement against a number 4.86% off. ✅ **And it closes what [E42](#errata-after-revision-9--change-request-cr-01-and-its-review) could not show**: the modulo bias contributes 1.7 × 10⁻⁹ relative and **does not reach the security bound at all.**

**[RT-20a](#9-conformance) also could not run where this document promised it could.** [11.11c](#1111-where-x25519-comes-from) made the derivation pure so that the row would need no curve — and the row still had to *compute four shared secrets*. [§10.4](#104-guided-pairing) now publishes an **interposer quadruple with both legs' `Z`**, so the assertion is deterministic and needs no key agreement at all. Its statistical half is restated as *no collision, and uniformity by χ²* — never the rate, because separating 10⁻⁶ from a 5% neighbour needs of order 10⁹ trials for 1.5 σ, and **a row nobody can run is a row that gets ticked.**

**And [E53](#errata-after-revision-9--change-request-cr-01-and-its-review) had not been carried everywhere it reached.** [3.4d3](#34-resolvable-identifiers) still argued from bays. It survives the correction on a better reason than the one it was given: **[7.4a](#74-persistent-pairings) gives a persisted pairing no expiry**, so a studio host accumulates them indefinitely and the count that sets the reconnection wait is the accumulated total, not the devices in the room.

**[B14](#annex-b--open-issues), [B15](#annex-b--open-issues) and [B18](#annex-b--open-issues) close.** B14 on hardware at both ends — CryptoKit on an iPhone 16, small-order keys rejected by throwing, [5.4b](#54-resolved-the-mechanism)'s rule satisfied rather than waived. B15 and B18 as decisions rather than questions.

⛔ **CR-01 has no open specification items.** What remains is evidence: [B14](#annex-b--open-issues) is discharged, [RT-20b and RT-20c](#9-conformance) wait for code, and [9g](#9-conformance) already forbids anyone reporting a pass without them.

## The premise did not hold — E53

**[CR-01](../changerequests/CR-01-in-band-pairing.md) was granted on a deployment that does not exist.** The request asked for a first pairing without a code *"in a venue where a range operator sets up several bays"*. The host is never at the range. A capture device there works standalone and its session travels home as a bundle; the two peers meet on a home or coaching-studio network.

**Everything that followed from the venue premise is withdrawn**, and none of it was a clause: [3.6a](#36-multicast-is-not-to-be-relied-on)'s *"it will not work at a range"* is a true statement about multicast that never bounded this feature; [3.7h](#37-the-bootstrap-window) is an ordinary MAY and not the item deciding whether RV-6 was worth building; [B15](#annex-b--open-issues)'s fleet case is a studio's two or three stereo devices; [3.4d3](#34-resolvable-identifiers)'s rotation is sized for a handful of pairings. **The mechanism is untouched** — nothing in [§11](#11-rv-6--guided-pairing) ever depended on where the peers were standing.

**RV-6 is kept, and the honest justification is narrower than the one it was granted on.** Comparing six digits needs no camera, no focus and no line of sight between two screens, and works when one of them is a monitor that cannot be pointed at anything. That is a real improvement on scanning a code for one phone and one PC in one room, which is the deployment that exists. It is not the multi-bay operator story, and the record should not go on implying that it is.

⛔ **The lesson is the part worth keeping.** RV-6 was reviewed four times by two teams and produced twenty findings — [E34](#errata-after-revision-9--change-request-cr-01-and-its-review) and [E43](#errata-after-revision-9--change-request-cr-01-and-its-review) among them, both structural, both invisible in the vectors — and **not one finding asked whether the deployment the feature was for existed.** Both teams inherited the premise from the request; so did the owner. Three passes then escalated [3.7h](#37-the-bootstrap-window) as the thing that decided whether the feature was worth building, on the strength of it. **The reviews checked the answer, with great care, and never checked the question.**

## The CR-01 fourth review pass — errata E49–E51, and RT-20 answered

Both teams: **ready to implement**, no objection to starting. Five findings, three of them against the API note rather than the specification — which is what that note is for.

**[RT-20](#9-conformance) is answered twice over: the test was badly formed, *and* splitting it needed a guard.** It had been reported as *not moved* in four consecutive passes, and the reason is now plain: it was **one row bundling three claims of different kinds.** *"No single-implementation harness can run it"* was true of the row and false of two thirds of its contents. The interposition property is **arithmetic** — [11.11c](#1111-where-x25519-comes-from) made the derivation a pure function, so four keypairs and two derivations settle it, with no socket and no counterpart, **before any of [§11](#11-rv-6--guided-pairing) is written**; and [E54](#errata-after-revision-9--change-request-cr-01-and-its-review) then made it deterministic against a published quadruple, so it needs no curve either. The behaviour — including [11.5c](#115-the-exchange)'s ordering, the clause everything rests on — needs **one** implementation and the relay, which observes it directly by withholding `bs_reveal`. Only the interop result needs two. **RT-20c is still the conformance claim and a and b passing is not it**; what changes is that a peer can state precisely what it has shown instead of reporting one row as unrun for the whole of development.

⛔ **Splitting RT-20 adds green, and green was half the objection.** Every other test in the list checks that two *honest* implementations compute the same numbers; RT-20 is the only one in which somebody is attacking. Four vector reproductions, two implementations, four passes and twenty findings is a great deal of verification, **none of it touching the security property** — and [11.1d](#111-what-this-path-is-and-the-one-thing-it-cannot-be) already names the extreme: a peer that quietly compares the digits itself passes every static test here while authenticating nothing. [9g](#9-conformance) is the guard: **a claim names [RT-20c](#9-conformance) and states its result, and no aggregate pass may be reported while it is unrun.** What each part does *not* prove is now tabulated beside it — including that RT-20c shows the protocol emits a mismatch signal and **not** that a tired operator notices it, which no protocol test reaches. And the relay is stated as the **first** artefact, which follows from the table: both security-touching tests depend on it, and it is the only thing in the list that needs no application to exist.

**[R-19](#errata-after-revision-9--change-request-cr-01-and-its-review) is the sharpest objection in four passes and it is against [E47](#errata-after-revision-9--change-request-cr-01-and-its-review), which is mine.** E47 told a multi-pairing advertiser to rotate every few seconds and described the cost as *"only multicast chatter"*. [3.2a](#32-instance-name) ties the instance name to `rid`, so every rotation **renames the service** — deregister, probe, announce — and at that rate it is continuous traffic on precisely the networks [3.6a](#36-multicast-is-not-to-be-relied-on) says rate-limit and drop multicast. **The mitigation for a discovery problem would have triggered the condition that breaks discovery.** E47 also claimed faster rotation was *"strictly better for unlinkability"*, and it is not: the address and port are constant across a registration, so an observer links the rotations by inspection. [3.2d](#32-instance-name) fixes the first — a stable per-registration random name, rotating only the TXT, one announcement instead of a cycle — and the second claim is simply withdrawn.

**[E51](#errata-after-revision-9--change-request-cr-01-and-its-review) is the same shape as [E40](#errata-after-revision-9--change-request-cr-01-and-its-review) and [E43](#errata-after-revision-9--change-request-cr-01-and-its-review), one layer down.** [11.6f](#116-derivation)'s prose carried the failure case and its **list** did not — and a list is what an implementer works from. A peer holds `PRK` from the moment it has `Z`, for up to the sixty seconds [11.3e](#113-roles-and-the-connection) allows a user to affirm, so every abort path left key material for a pairing that never existed.

**Both teams corrected their own earlier work again**, which is now the pattern of this review rather than an incident: PinPointCapture withdrew its third-pass sentence that *"the silent failure class has exactly two members"* as unscoped, and confirmed that [E43](#errata-after-revision-9--change-request-cr-01-and-its-review) was a stronger finding than the precision point it had itself raised.

## B17 closed — where X25519 comes from (E48)

**Not a review finding: an open issue answered.** [B17](#annex-b--open-issues) asked where the one primitive this protocol set has never needed before should live, given that one of the two implementations of this document carries no cryptographic library by construction and cannot acquire one.

**It is a parameter, not a callback**, and the reasoning is the shape of the two things rather than a preference. A callback is right where the computing component needs something it cannot produce *during a loop it owns* — which is why [5.3a1](#53-psk-identity)'s rejection sampling takes one. Key agreement has no loop: the component holding the private scalar already has it and already has the counterpart's public key, so it computes `Z` once and hands over 32 octets, exactly as it already hands over `psk`, `sid` and `rn`.

**[§11.11](#1111-where-x25519-comes-from) states it in octets rather than as an interface**, which is what keeps it language-neutral and leaves each implementation its own shape. The clause with teeth is [11.11f](#1111-where-x25519-comes-from): [E36](#errata-after-revision-9--change-request-cr-01-and-its-review) established that one library rejects a small-order key by *failing the call* and another by *throwing*, while a third might return zeros — and across a boundary **each side can see only one of those**. Both map to `invalid_key`, neither is a transport error, and neither is retried.

**The gain is bigger than tidiness.** [11.11c](#1111-where-x25519-comes-from) makes the whole derivation a pure function of `Z` and the transcript, so **[§10.4](#104-guided-pairing) can be reproduced, and [RT-18](#9-conformance) run, by a component that cannot do elliptic-curve arithmetic at all** — in the one component both applications share, rather than twice over in two applications that would then be agreeing with themselves. That is [B7](#annex-b--open-issues)'s trap, avoided by where the boundary was put.

⛔ **Nothing is implemented and no session is open.** The programme closed on 23 August 2026 and stays closed; the API is [recorded](../changerequests/CR-01-x25519-seam.md) so that three teams do not design one boundary three ways whenever it is built.

## The CR-01 third review pass — errata E43–E47, and question 3 answered

Both teams re-read **only the text no reviewer had seen** — E40–E42 and the owner's own unreviewed additions — as asked. Both return **ready to implement**. Five findings; none touches the wire, the vectors or the security argument; none reopens the design.

**[R-11](#errata-after-revision-9--change-request-cr-01-and-its-review) is the one that had to be fixed, and it is the third instance of one pattern.** [E40](#errata-after-revision-9--change-request-cr-01-and-its-review)'s rationale claimed *"`Z` already commits to both public keys by construction"*. **X25519 is not contributory**: clamping forces every scalar to a multiple of 8, so a counterpart can present a different public key that yields a bit-identical, non-zero shared secret — verified on two implementations, one of them OpenSSL, which accepted the substituted key without complaint. Follow it through and `BK`, `sid` and `PRK` are all identical under the substitution; **the only thing separating the two peers is the explicit `pk_i || pk_a` in `sas_raw`** — the binding the sentence described as redundant.

So a reader who believed it had been handed the argument for deleting the one clause holding the property up. That is [11.5c](#115-the-exchange)'s shape — a removal that reads as a tidy-up and is fatal — **arriving through a rationale rather than a clause for the third pass running**: [E37](#errata-after-revision-9--change-request-cr-01-and-its-review) inverted why a MAC fails, [E40](#errata-after-revision-9--change-request-cr-01-and-its-review) over-generalised what binds to what, [E43](#errata-after-revision-9--change-request-cr-01-and-its-review) asserted a property a primitive does not have. **Every clause in this section has now been reviewed more carefully than the sentences explaining them**, and the explanations are what an implementer reads first.

**[R-15](#errata-after-revision-9--change-request-cr-01-and-its-review) is the third instance of the *other* pattern** — a rule two implementations could resolve differently with neither wrong, and which [§10.4](#104-guided-pairing) can never expose, after [E41](#errata-after-revision-9--change-request-cr-01-and-its-review)'s *which `v` is bound* and [E39](#errata-after-revision-9--change-request-cr-01-and-its-review)'s role question. Unknown map keys are now **rejected**, which closes it and deliberately makes [B18](#annex-b--open-issues) a v2-from-scratch matter.

**[R-12](#errata-after-revision-9--change-request-cr-01-and-its-review) and [R-13](#errata-after-revision-9--change-request-cr-01-and-its-review) fix the diagnostics rather than the design**, and both matter more than their size: §10.4's triage list named one silent failure where there are two, and [RT-24a](#9-conformance) as a `static` row asserted nothing [RT-18](#9-conformance) did not. §10.4 now publishes **counter-vectors** — what a *wrong* implementation produces — which is what makes a reproduction self-checking, and is the same argument that put the little-endian misread in the list.

**[R-14](#errata-after-revision-9--change-request-cr-01-and-its-review): the version gap is real, and both teams argued against fixing it with negotiation.** A newer initiator cannot pair with an older acceptor, and on this deployment that is the normal ship order. Adding agility to a first-contact handshake to close a gap whose fallback is the REQUIRED path would trade a permanent attack surface for a temporary inconvenience. So the fallback is made immediate ([11.9d1](#119-aborting-and-the-one-attempt-rule)), the ship order is written down, and [B18](#annex-b--open-issues) records the question with the constraint on any future answer.

**[E42](#errata-after-revision-9--change-request-cr-01-and-its-review)'s attribution was wrong and is corrected.** The bad arithmetic came from PinPointCapture's first pass, not PinPointStudio's — whose figure was right. PinPointStudio accepted the blame in pass 2 without checking, and **PinPointCapture corrected the record against itself in pass 3.** An erratum about miscopied numbers had miscopied whose they were.

**Question 3 is answered.** The protocol owner has committed the deployment to host-advertises / device-dials, and [A18](#annex-a--decisions-and-alternatives) records the reasoning as a decision rather than a concession: a foregrounded application dials reliably where a backgrounded one cannot listen, and the host is set up first and already advertising. **The platform constraint and the ergonomics agree.** That answer then made a latent problem live — [E47](#errata-after-revision-9--change-request-cr-01-and-its-review): [E27](#errata-after-revision-8)'s mitigation for the multi-pairing reconnection wait assumes the multi-pairing peer is the one that can *browse*, and this deployment inverts it.
...
⚠ *The table continues past CR-01.* **[E56](#errata-after-revision-9--change-request-cr-01-and-its-review) has nothing to do with the change request** — it is an ordinary implementation finding against [§7.2](#72-handling-the-pairing-secret) that happened to land after CR-01 closed. It is recorded here because the errata sequence is one sequence and this is the table it belongs in, not because the heading describes it.
...
| **E42** | [§11.7](#117-the-short-authentication-string), [§10.4](#104-guided-pairing) | **Corrected, 24 August 2026 — findings R-07 and R-08 (PinPointStudio) and F-R9-3 (PinPointCapture). Prose only; no clause, vector or security claim changes.** Two arithmetic errors in published commentary, both in notes whose purpose is to stop a reader making an arithmetic error. The modulo-bias paragraph claimed `295 967 296` residues out of 10⁶, put the favoured set the wrong way round, and understated the excess over uniform by **33×**. [§10.4](#104-guided-pairing)'s little-endian example was mis-reversed at source *and* computed from the pre-E34 `sas_raw`, three lines below the box warning against exactly that. Two of the three figures originated in **PinPointCapture's** pass-1 review and were quoted onward in good faith — *corrected at the third pass, having been attributed to PinPointStudio here and accepted by them in pass 2 without either party checking*; PinPointStudio's own figure was right. **Both teams recomputed every row of the vector and neither recomputed the numbers in the prose beside them.** |
| **E43** | [11.6c2](#116-derivation), [11.6c1](#116-derivation), [§10.4](#104-guided-pairing) | **Amended, 24 August 2026 — finding R-11, PinPointStudio; independently the precision point in PinPointCapture's third pass. The one to fix before implementation.** [E40](#errata-after-revision-9--change-request-cr-01-and-its-review)'s rationale rested on *"`Z` already commits to both public keys by construction"*, which is **false**: X25519 is not contributory — clamping makes every scalar a multiple of 8, so a counterpart can offer a **different** public key yielding a **bit-identical, non-zero** shared secret, and [11.6b](#116-derivation) does not fire. §10.4 now carries the witness. The clause was right and the reason was not, and **the reason pointed at the one thing holding the property up**: under that substitution `BK`, `sid` and `PRK` are all identical, and only `sas_raw`'s explicit `pk_i \|\| pk_a` separates the peers. A reader believing the claim had been handed the argument for simplifying that binding away — [11.5c](#115-the-exchange)'s shape arriving through a rationale, which is [E37](#errata-after-revision-9--change-request-cr-01-and-its-review) one pass earlier and [E40](#errata-after-revision-9--change-request-cr-01-and-its-review) one pass after. Claim deleted; 11.6c2 forbids the simplification explicitly. **Not an attack, and no new check is wanted.** |
| **E44** | [§10.4](#104-guided-pairing), [RT-24a, RT-24b](#9-conformance) | **Amended, 24 August 2026 — findings R-12 and R-13 (PinPointStudio) and F-R9-4 (PinPointCapture).** §10.4 claimed the sixth divergence cause was the **only** one producing a successful-looking pairing; **cause 1 has the identical signature** — `sid` feeds only `PRK`, so both agree on the digits and both MACs and diverge only at the TLS handshake. In a paragraph whose entire job is triaging `PSK_IDENTITY_NOT_FOUND`, that routed the reader away from the older and more familiar cause. **Counter-vectors added** — the two wrong values, and the R-11 witness — so a reproduction is self-checking rather than merely matching. [RT-24a](#9-conformance) becomes **`review`**: as a `static` row it asserted nothing [RT-18](#9-conformance) did not, since a bound transcript changes `PRK` and RT-18 already fails. |
| **E45** | [11.9d1](#119-aborting-and-the-one-attempt-rule), [B18](#annex-b--open-issues) | **Added, 24 August 2026 — finding R-14, PinPointStudio; endorsed by PinPointCapture.** A peer aborting `unsupported_version` offers the pairing code on the **first** abort, not the second: [11.4h](#114-frames) guarantees the retry fails identically, so [11.9d](#119-aborting-and-the-one-attempt-rule) spent an operator's attempt on a certainty. This bounds §11's one real version gap — a **newer initiator cannot pair with an older acceptor**, and on this deployment that is the normal ship order — to *pairs by code, this once*. The ship order (**acceptor side first**) is recorded, and negotiation is recorded at [B18](#annex-b--open-issues) as deliberately unanswered, with the reasoning both teams gave for not adding it. |
| **E46** | [11.4c1](#114-frames) | **Added, 24 August 2026 — finding R-15, PinPointStudio.** A peer **rejects an unrecognised map key** in a bootstrap frame. [11.4c](#114-frames) enumerated unknown frame *types* and said nothing about keys, while [3.3a](#33-txt-record), [4.2c](#42-version-handling) and [A4](#annex-a--decisions-and-alternatives) all point the other way — so two implementations could differ with neither wrong, and [§10.4](#104-guided-pairing) could never show it. The **third** instance in three passes of the class a vector cannot catch. Rejecting matches [11.10a](#1110-what-must-not-cross-a-bootstrap-connection) and the fail-closed channel byte, and it makes [B18](#annex-b--open-issues) a v2-from-scratch matter — accepted deliberately and stated rather than discovered. |
| **E47** | [3.4d3](#34-resolvable-identifiers) | **Added, 24 August 2026.** A multi-pairing **advertiser** whose counterparts cannot advertise ([3.5d](#35-who-advertises-and-who-browses)) rotates far faster than [3.4a](#34-resolvable-identifiers)'s 15-minute floor, which is a floor on *frequency* and not a ceiling. [3.4d1](#34-resolvable-identifiers) bounds the reconnection wait to *rotation × pairings held*, and [E27](#errata-after-revision-8)'s mitigation for it — [3.4d2](#34-resolvable-identifiers)'s *browse as well as advertise* — **assumes the multi-pairing peer is the one that can browse. This deployment inverts it**: the host accumulates the pairings *and* must advertise, and browsing finds nothing because 3.5d silenced its counterparts. At the floor with ten pairings the wait reaches two and a half hours; at twenty seconds it is under four minutes. Raised by the protocol owner once [question 3](#35-who-advertises-and-who-browses) was answered, which made the case live. |
| **E48** | [§11.11](#1111-where-x25519-comes-from), [RT-27](#9-conformance), [B17](#annex-b--open-issues) | **Added, 24 August 2026 — closing [B17](#annex-b--open-issues), raised by PinPointStudio.** Where X25519 comes from, stated **in octets rather than as an interface**: exactly two 32-octet values — a peer's own `pk` and the shared secret `Z` — and **nothing else crosses**, because the boundary is *below* the derivation rather than inside it. It is a **parameter, not a callback**: key agreement has no loop, so the component holding the private scalar computes `Z` once and hands it over exactly as it already hands over `psk`, `sid` and `rn`. The consequence worth having is [11.11c](#1111-where-x25519-comes-from) — the derivation is a pure function of `Z` and the transcript, so [§10.4](#104-guided-pairing) is reproducible, and [RT-18](#9-conformance) runnable, by a component with no curve arithmetic at all; which puts it in the one component both applications share rather than twice in two that would then agree with themselves ([B7](#annex-b--open-issues)). [11.11f](#1111-where-x25519-comes-from) is the clause with teeth: [E36](#errata-after-revision-9--change-request-cr-01-and-its-review)'s two behaviours **split across the boundary** — one side can only see a failed call, the other only an all-zero `Z` — so both must map to `invalid_key`, neither to a transport error, and neither retried. |
| **E49** | [3.2d](#32-instance-name), [3.4d3](#34-resolvable-identifiers) | **Added and corrected, 24 August 2026 — finding R-19, PinPointStudio.** [E47](#errata-after-revision-9--change-request-cr-01-and-its-review) required a multi-pairing advertiser to rotate far faster than the floor and called the cost *"only multicast chatter"* and the effect *"strictly better for unlinkability"*. **Both were wrong.** [3.2a](#32-instance-name) ties the instance name to `rid`, so each rotation **renames the service** — a deregister/probe/announce cycle, not a TXT update — and at seconds-scale that is continuous traffic on exactly the networks [3.6a](#36-multicast-is-not-to-be-relied-on) says drop multicast: **the mitigation for a discovery problem would have triggered the condition that breaks discovery.** And within one registration the address and port do not change, so an observer links every rotation by inspection; faster rotation is **neutral** for unlinkability, not better — [3.4e](#34-resolvable-identifiers)'s cross-venue property is what rotation buys and it is unaffected. [3.2d](#32-instance-name) is the fix: a **stable per-registration random instance name**, rotating only the TXT, which costs one announcement. A browser is unaffected — [3.4b](#34-resolvable-identifiers) resolves `rid` from the TXT and never from the name. |
| **E50** | [RT-20a, RT-20b, RT-20c](#9-conformance), [RT-24b, RT-24c](#9-conformance) | **Amended, 24 August 2026 — finding R-20, PinPointStudio, and the standing report that [RT-20](#9-conformance) had not moved in four passes.** RT-20 was **one row bundling three claims of different kinds** — a computation, a per-peer behaviour, and an interop result — so the whole of it waited on the last of the three, and was reported unrun for the whole of development while two thirds were runnable far earlier. **That is a defect in the test, not in the protocol.** Split: **RT-20a** is arithmetic over four keypairs and runs before any of §11 exists, [11.11c](#1111-where-x25519-comes-from) having made the derivation pure — and a large run **measures** [§11.8](#118-what-the-comparison-proves)'s 2⁻²⁰ bound rather than restating it; **RT-20b** needs **one** implementation and the relay, and covers [11.5c](#115-the-exchange)'s ordering, which a relay observes directly by withholding `bs_reveal`; **RT-20c** is the interop claim and is the only part that needs both. RT-24b likewise splits — its R-11 witness needs an actual X25519 call, which [§11.11](#1111-where-x25519-comes-from) means the deriving component may be unable to make. **RT-20c remains the conformance claim; a and b passing is not it.** |
| **E51** | [11.6f](#116-derivation), [11.11h1](#1111-where-x25519-comes-from) | **Added, 24 August 2026 — finding F-R9-5, PinPointCapture.** 11.6f's closing sentence always carried the failure case and **its explicit list did not** — and the list is what gets implemented from. A peer computes the whole chain the moment it holds `Z`, before either user has affirmed and with [11.3e](#113-roles-and-the-connection) allowing sixty seconds for that, so every abort path left a `PRK` in memory for a pairing that does not exist. The list now names `PRK`, `K_tls`, `K_id` and `sid` on the failure path. 11.11h1 records that on at least one platform the erasure obligation is **partly on a closed-source framework** that neither exposes a zeroise nor documents one — the same class as [§5.4](#54-resolved-the-mechanism)'s limits: not a defect, a bounded truth about what a MUST can mean. |
| **E52** | [9g](#9-conformance), [RT-20](#9-conformance) commentary | **Added, 24 August 2026 — PinPointStudio, fourth pass, on the weight of accumulated green.** A conformance claim to [§11](#11-rv-6--guided-pairing) **names [RT-20c](#9-conformance) and states its result**, and **no aggregate pass** may be reported while it is unrun. Every other test in the list checks that two **honest** implementations compute the same numbers; RT-20 is the only one in which somebody is attacking, and the sheer volume of the rest is the hazard — four vector reproductions, two implementations, four passes, twenty findings, **and none of it touching the security property.** [11.1d](#111-what-this-path-is-and-the-one-thing-it-cannot-be) already names the extreme case: a peer that quietly compares the digits itself passes every static test here while authenticating nothing. Splitting RT-20 under [E50](#errata-after-revision-9--change-request-cr-01-and-its-review) added *more* green and would have made this worse without 9g. Also records what each part does **not** prove — including that RT-20c shows the protocol emits a mismatch signal and **not** that a tired operator notices it, which is human factors and which no protocol test reaches — and that the relay is the **first** artefact rather than the last, since both security-touching tests depend on it and it needs no application to exist. |
| **E53** | [§11](#11-rv-6--guided-pairing) preamble, [3.7h](#37-the-bootstrap-window), [3.4d3](#34-resolvable-identifiers), [B15](#annex-b--open-issues) | **Premise corrected, 24 August 2026 — protocol owner. Non-normative throughout; no clause changes.** [CR-01](../changerequests/CR-01-in-band-pairing.md) was raised for *"a venue where a range operator sets up several bays"*, and that deployment **does not exist**: the host is never at the range. A capture device there works standalone and its session travels home as a bundle; the two peers meet on a **home or coaching-studio network**, which the user controls and where multicast behaves. So [3.6a](#36-multicast-is-not-to-be-relied-on)'s *"it will not work at a range"* never bounded this feature, [3.7h](#37-the-bootstrap-window) is an ordinary MAY rather than the clause deciding whether RV-6 was worth building, [B15](#annex-b--open-issues)'s fleet case is a studio's two or three stereo devices rather than a venue's bays, and [3.4d3](#34-resolvable-identifiers)'s rotation is sized for a handful of pairings rather than ten. ⛔ **RV-6 is kept, on a narrower basis than it was granted on**: comparing six digits needs no camera, no focus and no line of sight, which is a real improvement on scanning a code even for one phone and one PC in one room. **Four review passes and twenty findings never asked whether the deployment existed** — both teams inherited the premise from the request, and so did the owner. |
| **E54** | [7.1](#71-threat-model), [§11.8](#118-what-the-comparison-proves), [§10.4](#104-guided-pairing), [RT-20a, RT-20b](#9-conformance) | **Corrected, 24 August 2026 — findings R-21/R-22/R-23 (PinPointStudio) and F-R9-6/F-R9-7 (PinPointCapture), the first two raised independently and computed identically.** **The bound is 1 in 1 000 000, not 1 in 1 048 576.** 2⁻²⁰ is the width of `sas_raw`; [11.7a](#117-the-short-authentication-string) reduces it mod 10⁶ before anyone sees it, and an attacker must match the **digits**. Exactly `144115188323/144115188075855872` = 1.0000000017 × 10⁻⁶ — **4.86% weaker than stated**, immaterial to the argument and material to a conformance row told to check a measured rate against it. It also settles what [E42](#errata-after-revision-9--change-request-cr-01-and-its-review) could not: the modulo bias contributes 1.7 × 10⁻⁹ relative and **does not reach the bound at all**. [RT-20a](#9-conformance) becomes **deterministic** against a published **interposer quadruple** — both legs' shared secrets are now in [§10.4](#104-guided-pairing), so the one test touching the security property needs no key agreement, which [11.11c](#1111-where-x25519-comes-from) had promised and this row had not delivered. Its statistical half becomes *assert no collision and assert uniformity by χ²*, never the rate: separating 10⁻⁶ from a 5% neighbour needs ~10⁹ trials for 1.5 σ, and **a row nobody can run is a row that gets ticked**. [RT-20b](#9-conformance)(ii) gains the **initiator mirror**, since [9e1](#9-conformance) lets a peer claim one role and one implementation here claims each. |
| **E55** | [3.4d3](#34-resolvable-identifiers) | **Amended, 24 August 2026 — PinPointStudio.** [E53](#errata-after-revision-9--change-request-cr-01-and-its-review) is carried into this clause's body, which still argued from a venue where devices move between bays. **The clause survives the premise, on a better reason:** [7.4a](#74-persistent-pairings) gives a persisted pairing **no expiry** — it ends on revocation and nothing else — so a coaching-studio host **accumulates pairings indefinitely**, and the count setting the reconnection wait is that accumulated total rather than the devices in the room. Size the rotation on **pairings held**, not devices present. [7.4b](#74-persistent-pairings)'s visible, individually revocable persistence is the only pruning mechanism, and it is a user action. |
| **E56** | [7.2c](#72-handling-the-pairing-secret), [7.1](#71-threat-model), [§7.4](#74-persistent-pairings) closing, [RT-12](#9-conformance) and its commentary | **Amended, 25 August 2026 — both implementations, and the first erratum after [CR-01](../changerequests/CR-01-in-band-pairing.md) closed. Unrelated to it.** **[7.2c](#72-handling-the-pairing-secret) becomes a SHOULD.** As a MUST it decided, on the consuming application's behalf, to spend a **user-facing permission** — which [§1.3](#13-where-it-stops) lists among the things this document explicitly does not decide, alongside *"how a peer stores its own secrets at rest"*. The clause reached into its own stated out-of-scope and settled it. ⛔ **It was breaking [§7.4](#74-persistent-pairings) and [§7.5](#75-reconnecting-within-a-session) in both implementations, from opposite directions.** One has a keychain on **one of its three platforms** and, reading *"where one exists"* honestly, offered **no persistence at all** on the other two: a paired peer forgotten on every launch, and reconnection with nothing to resolve. The other holds secrets in a class **unreadable while the device is locked**, so the reconnection sweep read nothing and reported *no pairings held* — false, and indistinguishable from the true statement. **This is [E3](#errata-after-revision-9--change-request-cr-01-and-its-review)'s shape**: a clause read correctly and literally, making a section of this document unreachable, found only when somebody implemented it. ⚠ **What is given up is stated rather than assumed away**: a `PRK` in ordinary settings is readable by anything running as that user, so [7.4a](#74-persistent-pairings) is a weaker offer than it was and [7.4b](#74-persistent-pairings)'s opt-in, visible, individually revocable persistence now carries the weight 7.2c used to share. **No compensating MUST is added** — deliberately, because the alternative was to re-specify the storage decision this erratum exists to hand back. ⚠ **Risk accepted by the maintainer, 25 August 2026**, on the reasoning that an attacker with access to the disk can read the data at rest anyway, with **full-disk encryption** as the deployment mitigation ([§8](#8-operational-notes)). ⛔ **That reasoning covers most of the exposure and not all of it**, and the two exceptions are recorded so the acceptance is not read wider than it was given: a `PRK` read from one device authenticates to its **counterpart**, whose sessions were never on the compromised disk; and with no forward secrecy at the TLS 1.2 PSK floor, traffic **recorded earlier** decrypts once the key is obtained at any later date, having never been at rest on that disk at all. Encrypting the volume reduces both and removes neither — an attacker running as the user reads through it. |
| **E57** | [7.4b](#74-persistent-pairings), [7.1](#71-threat-model) threat model, [5.4h](#54-resolved-the-mechanism), [3.4d3](#34-resolvable-identifiers), [§8](#8-operational-notes) | **Amended, 25 August 2026 — the protocol owner. Same movement as [E56](#errata-after-revision-9--change-request-cr-01-and-its-review), the same day.** **7.4b becomes a SHOULD.** ⛔ **The objection is to *opt-in* specifically: a user deliberately pairing a device has already said what they want**, and requiring a second agreement before the pairing may be remembered puts a consent step inside a flow whose whole purpose was consent — friction bought with nothing. Both implementations hit the predictable result: a default-off toggle on one screen of two, and a paired device silently forgotten at the end of the session. **And the general point decides it:** it is overstepping for a protocol to tell an application how to behave. Opt-in, visible and individually revocable is a statement about **screens**, `RV` §1.3 already puts *user interface* out of scope, nothing in the clause is observable between peers — two implementations, one with a revocation list and one without, produce **byte-identical** traffic — and the reference library cannot implement a line of it. ⚠ **What it costs, because [E56](#errata-after-revision-9--change-request-cr-01-and-its-review) leaned on this clause by name:** E56 relaxed 7.2c partly on the reasoning that 7.4b remained a MUST and carried the weight. **After E57 nothing normative protects a persisted `PRK`** — not where it is stored, not that a user can see or remove it. ⛔ **The sharp edge:** 7.4d is unchanged and still a MUST, so a revocation *is* honoured immediately — but nothing now requires that a means of revoking **exists**, and 7.4a gives a persisted pairing no expiry. What still binds is narrow: 7.4c scopes it to the counterpart, 7.4f forbids persisting from a multi-use code. The clause is **kept as advice, not deleted** — it is still the better product. |
