...
| Revision | 9 — adds RV-6 (guided pairing), plus errata E34–E52 from four review passes over it and the closure of B17. Revision 8 was final; [CR-01](../changerequests/CR-01-in-band-pairing.md) reopened it. |
...
| **RT-20a** | static | *Erratum E50 — the interposition property as **arithmetic**.* Four keypairs — an initiator, an acceptor, and the two an interposer would hold — and both legs' digits computed through [11.6c](#116-derivation): `derive(X25519(sk_i, pk_M1), v, pk_i, pk_M1)` against `derive(X25519(sk_M2, pk_a), v, pk_M2, pk_a)`. **Assert they differ.** Over a large run of random quadruples, assert the collision rate matches [§11.8](#118-what-the-comparison-proves)'s 2⁻²⁰ within sampling error — which **measures** the bound this section rests on rather than restating it. Needs no socket, no peer and no counterpart, and runs in whichever component holds the derivation ([11.11c](#1111-where-x25519-comes-from)). |
| **RT-20b** | injected | *Erratum E50 — the behaviour, against **one** implementation.* One real peer, the relay on the other side. Assert: **(i)** the digits it displays are the ones [11.6c](#116-derivation) gives for its own leg; **(ii)** an acceptor sends `bs_accept` **before** it has received `pk_i` — [11.5c](#115-the-exchange)'s ordering, **the clause the entire property rests on**, and directly observable by a relay that withholds `bs_reveal` and checks `pk_a` already arrived; **(iii)** a user who declines does not pair; **(iv)** the window closes and does **not** reopen without a further user action ([3.7b](#37-the-bootstrap-window), [11.9b](#119-aborting-and-the-one-attempt-rule)); **(v)** each of the relay's own legs completes on demand, or the harness is testing its own bug. |
...
| | Needs | Runnable |
|---|---|---|
| **RT-20a** | the derivation, and nothing else | **Before any of [§11](#11-rv-6--guided-pairing) is implemented.** [11.11c](#1111-where-x25519-comes-from) makes the derivation a pure function, so the interposition property is arithmetic over four keypairs — and the 2⁻²⁰ bound becomes something a run **measures** rather than something the document asserts |
...
|---|---|---|
| **RT-20a** | The *derivation* separates an interposer's two legs, and the 2⁻²⁰ bound is the real number | Anything about an implementation. It never runs against a peer, so **a peer that auto-confirms passes it trivially** ([11.1d](#111-what-this-path-is-and-the-one-thing-it-cannot-be)) |
...
⚠ **Three review passes, not five.** [§4](#4-rv-2--the-pairing-code) survived three passes and three independent recomputations of its vectors; this has now had three of each — and each pass found a defect the previous pass's own fix had introduced ([E40](#errata-after-revision-9--change-request-cr-01-and-its-review), then [E43](#errata-after-revision-9--change-request-cr-01-and-its-review)). Its vectors now reproduce byte for byte on two implementations sharing no code, on both sides of the E34 boundary. **[RT-20](#9-conformance) — the test the path exists to pass — still cannot run**, because it needs two real implementations either side of a deliberate relay and neither has written this section yet. Nothing in two review passes has changed that, and it should not be allowed to feel as though it has.
