...
| **CT-I39** | I39 | Actuate | static | *Erratum E58, CR-02.* For an Actuator declared `control: on_off`, assert `actuator_command` carrying `level` (with or without `on`) is rejected `malformed`, and one carrying `on` alone is accepted. For one declared `control: level`, assert the reverse. Then, paired: send a well-formed command to an undeclared `actuator_id` and assert `error` / `not_declared` (12.1d); send one from a peer other than the Session's `role: host` and assert it is refused rather than acted on (12a). |
