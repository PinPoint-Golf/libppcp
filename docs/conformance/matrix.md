...
| RT-12 | **review** | CSPRNG at full width, erasure (**storage dropped by [E56](../specification/ppcp-core.md#errata-after-revision-9)** — `RV` 7.2c is a SHOULD; the reviewer records where secrets are held, and does not fail the row for a store that is not the platform's protected one) | H6, D7 | n/a | `maintainer-accepted` | `maintainer-accepted` |
...
| RT-17 | **review** | every platform mode offered, from a capability query | H1, D1 | n/a | `maintainer-accepted` | `maintainer-accepted` |
...
| RT-23 | **review** | ephemeral key, `Z`, `BK`, `K_c` erased on completion **and abort** | D11 | n/a | n/a | review — **readable**: erasure asserted on all four exit paths (abort, decline, timeout, wipe). Reviewer unassigned |
...
⚠ **The relay, running on the Mac, CAN dial a listener inside the iOS Simulator** — verified 24 August 2026. That was the open readiness question for [RT-20c](../specification/ppcp-rv.md#9-conformance) and it is **not** a blocker.

⛔ **The harness affirms in software, and that is trap 8 wearing a high-visibility jacket.** `make rv6`'s pairing mode prints `[AFFIRM-IN-SOFTWARE — not a conformant peer]` on every run, and the orchestrator confirmed the path exists **only in `Tests/GuidedPairingRelayTests.swift` and nowhere under `Sources/`**. A test must supply **the tap, never the comparison** ([11.1d](../specification/ppcp-rv.md#111-what-this-path-is-and-the-one-thing-it-cannot-be)) — a peer that compares the digits itself passes every static test in the document and authenticates nothing. **Any future appearance of this string outside a test target is a defect, not a convenience.**
...
**Reproducing PinPointCapture's RT-22**, both halves re-run by the orchestrator rather than taken from the agent's report: the `bs`/`rn`/`rid` key-set and the `bs`+`rid` rejection are `make test-core` — *"RV 3.7 — the bootstrap window"*, **254 tests in 31 suites**. Withdrawal on close is `make test-app` — *"RV §3.7 — the bootstrap window on a real socket"*, artefact `Test-PinPointCapture-2026.08.24_17-56-33.xcresult`, `"result": "Passed"`, **74 total, 0 failed**, all eight bootstrap cases passing.

⚠ **The cell is qualified and the qualification is the honest part.** [3.7b](../specification/ppcp-rv.md#37-the-bootstrap-window) withdraws the **instance** — a DNS-SD property. What is measured is that the **socket** stops answering: `close(on:)` does not return until `NWListener` reaches `.cancelled`. **The record-level assertion — the instance is gone from the responder — is not measured**, because it needs a live responder and the local-network permission; it stays open as PinPointCapture #66. A correct implementation could deregister promptly and hold a socket a moment; a broken one could drop the socket and leave the record advertised, **which is the worse failure and this test would not see it**.

✅ **This row failed first and passed second, and the first result was the useful one.** The withdrawal assertion's initial failure was a **real defect, not a flaky test**: `NWListener.cancel()` is asynchronous, `close(on:)` returned inside the gap, and a dial arriving there completed its TCP handshake into the accept backlog before being reset — logged as `SO_ERROR [54: Connection reset by peer]` at 17:25:12. A bootstrap endpoint answering after its window has closed is what [11.9b](../specification/ppcp-rv.md#119-aborting-and-the-one-attempt-rule) exists to prevent.

⚠ **Four rows here are `review` and that is not an accident.** RT-23, RT-24a, RT-25, RT-26 and RT-27 each catch something that produces **byte-identical handshakes** — a peer violating [11.3d1](../specification/ppcp-rv.md#113-roles-and-the-connection) is conformant on the wire, and one that compares the digits in software passes every static test in the document. They join [RT-12](../specification/ppcp-rv.md#9-conformance) and RT-17 in the set nothing external can check. **Each needs a named reviewer and a commit.** ✅ [RT-12](../specification/ppcp-rv.md#9-conformance) has one as of 25 August 2026 — `maintainer-accepted`, see below; the others do not.
...
✅ **An expert review is therefore still WANTED, and is no longer BLOCKING** — [`freeze-readiness.md`](freeze-readiness.md) moves it accordingly. ✅ **[RT-12](../specification/ppcp-rv.md#9-conformance) and [RT-17](../specification/ppcp-rv.md#9-conformance) were both discharged 25 August 2026** as `maintainer-accepted` — the maintainer read the generator and erasure paths in both applications. ⛔ **Recorded as `maintainer-accepted` and deliberately NOT as `pass`**: this project has never had a qualified cryptographic reviewer, and the owner accepting a risk is a different statement from an expert having checked it. What was read: one CSPRNG entry point per application over the platform's audited source (`getentropy`/`BCryptGenRandom`/`SecRandomCopyBytes`), **no fallback on any path**, `psk` drawn at 32 bytes against a 16-byte floor, and erasure via `OPENSSL_cleanse` on every exit path of the C++ side. **Two platform limits are accepted rather than fixed**, both unfixable in application code: Swift exposes no zeroise for `Data` or `CryptoKit`'s `SharedSecret` ([11.11h1](../specification/ppcp-rv.md#1111-where-x25519-comes-from) already records this), and removing a row from a file does not shred the superseded disk blocks.

✅ **[RT-17](../specification/ppcp-rv.md#9-conformance) — offering every mode the platform exposes — discharged the same day, also `maintainer-accepted`.** PinPointCapture enumerates ciphersuite groups from the SDK at run time and names no suite as a constant, and sets TLS 1.3 as a **maximum** so a platform that gains external PSK is used without an edit; it changes nothing today because iOS exposes no PSK suite at all, which is compliance **by construction** under [5.2i](../specification/ppcp-rv.md#52-tls-profile). PinPointStudio probes OpenSSL per suite and verifies each really landed in the list. ⚠ **One gap accepted rather than fixed**: Studio's *candidate list* is a constant, so it queries the availability of a known set rather than the set itself. It is complete against every PSK suite the RFCs define today, so there is no live defect. **The maintainer's reasoning, recorded so it need not be reconstructed:** *even if a new capability arrives later, the current code is robust enough given the data at risk* — a suite arriving unoffered costs a forward-secrecy improvement this deployment does not have today and has already accepted doing without ([§5.4.3](../specification/ppcp-rv.md#543-the-decision) made the same judgement on the same payload), and exposes nothing currently protected. The known fix, should the list fall behind, is a true enumeration via OpenSSL's `"PSK"` cipher-string alias.
