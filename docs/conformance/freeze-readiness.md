...
| Recommendation | **Freeze the documents. Do not declare `ppcp/1.0` stable yet.** See [§5](#5-recommendation) for the exact conditions |
...
| 3 | **RT-12 is discharged by a named reviewer, in both applications.** `RV` §9 calls it the requirement the whole security model rests on and the one no test can catch. It is currently discharged by nobody | The user, to assign | human review |
| 4 | **RT-17 is discharged by a named reviewer, in both applications** | The user, to assign | human review |
...
| 7 | ⛔ **`RT-24a` and `RT-27` are read by an independent, qualified reviewer.** *Added 24 August 2026, CR-01 session C1.* Both are **accepted by the maintainer, not discharged** — he read the reasoning for each, **stated that he is not qualified to approve a cryptographic review and that there is no one else to consult**, and accepted the residual risk on that basis. That is an honest record of an **unmet** requirement, and it must not be promoted to `pass`. RT-24a is the transcript-binding rule whose breach yields matching digits, matching MACs and a divergent `PRK` — surfacing as `PSK_IDENTITY_NOT_FOUND`, indistinguishable from the [3.5d](../specification/ppcp-rv.md#35-who-advertises-and-who-browses) platform limitation. RT-27 is the [§11.11](../specification/ppcp-rv.md#1111-where-x25519-comes-from) boundary, which is invisible on the wire by construction. See [`matrix.md` §5b](matrix.md#5b-two-review-rows-are-accepted-rather-than-discharged) | An independent reviewer, to find | human review |
